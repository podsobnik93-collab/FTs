...
function handleBuilderVoiceSearch() {
  ...
  // 1. Голос только с названием группы мышц (без цифр) — открываем сетку
  ...
  // 2. Голос с параметрами подхода ("3 по 10, 20 килограмм", "жим лёжа 4 по 8 60 кг")
  if (hasDigitsOrParams && typeof parseVoiceForBuilder === 'function' && typeof fuzzyMatchExercise === 'function') {
    ...
    // Если распознали цифры, но упражнение не нашли по имени — попробуем применить параметры
    // к уже выбранному упражнению группы, если такое есть
  ...
  // 3. Фолбэк — обычный текстовый поиск упражнения
  searchBuilderEx();
}
