...
function updateAddStartBar(){
  const bar=document.getElementById('bld-start-bar'),outerBtn=bar?bar.querySelector('button'):null,label=document.getElementById('bld-start-label');
  let total=0;for(const s of Object.values(addPickerChecked))total+=s.size;
  if(bar)bar.style.display=total?'block':'none';
...
function closeAddMode(){
  ...
  if(wtExecute)wtExecute.style.display='block';if(wtBuilder){wtBuilder.style.display='none';wtBuilder.classList.remove('has-muscle-picker');wtBuilder.removeAttribute('data-mode');const backBtn=wtBuilder.querySelector('.workout-builder-back');if(backBtn){backBtn.textContent='‹ Тренировка';backBtn.onclick=exitWorkoutBuilder;}}
