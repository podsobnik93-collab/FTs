...
function renderExPicker(cat){
  ...
  for(let i=0;i<exs.length;i++){
    ...
    html+=`<div class="ex-pick-row ${isChk?'row-checked':''}" data-i="${i}" data-cat="${cat}"><div class="ex-pick-row-top" onclick="togglePickEx(${i}, '${cat}')"><div class="ex-pick-check ${isChk?'checked':''}">${isChk?'✓':''}</div>${getExerciseImage(e.n)?`<img class="exercise-thumb" src="${getExerciseImage(e.n)}" alt="" loading="lazy">`:''}<div class="ex-pick-info"><div class="ex-pick-name">${e.n}</div>${isChk&&p.lastSetsData?`<div class="ex-pick-history">Прошлый раз: ${p.lastSetsData.map(x=>`${x.r}×${x.kg} кг`).join(' · ')}</div>`:''}</div></div>${paramsHtml}</div>`;
  }
...
function renderExecute() {
  ...
  if (!el) return;
  let html = '';
  customWt.forEach((e, i) => {
    if (!e.sets_data) e.sets_data = Array.from({length: e.s}, () => ({r: e.r, kg: e.kg, done: false}));

    let setsHtml = '';
    e.sets_data.forEach((set, si) => {
      setsHtml += `
        <div class="exe-set-row ${set.done ? 'done-set' : ''}">
          <div class="exe-set-info">
            <span class="exe-set-num">Подход ${si + 1}</span>
            <div class="exe-set-inputs">
              <div class="exe-val-box"><b>${set.kg}</b><span>кг</span></div>
              <div class="exe-val-box"><b>${set.r}</b><span>повт</span></div>
            </div>
          </div>
          <button class="exe-done-btn ${set.done ? 'done' : ''}" onclick="toggleSet(${i},${si})">${set.done ? '✓ Готово' : 'Готово'}</button>
        </div>
      `;
    });

    const editControls = editMode ? `
      <div class="execute-edit-controls">
        <button class="confirm-btn-cancel" style="padding:4px 8px;font-size:12px;min-height:30px" onclick="moveEx(${i},-1)" ${i===0?'disabled':''}>↑ Вверх</button>
        <button class="confirm-btn-cancel" style="padding:4px 8px;font-size:12px;min-height:30px" onclick="moveEx(${i},1)" ${i===customWt.length-1?'disabled':''}>↓ Вниз</button>
        <button class="confirm-btn-cancel" style="padding:4px 8px;font-size:12px;min-height:30px" onclick="addWorkoutSet(${i})">+ Подход</button>
        <button class="confirm-btn-cancel" style="padding:4px 8px;font-size:12px;min-height:30px" onclick="removeWorkoutSet(${i})" ${e.sets_data.length<=1?'disabled':''}>− Подход</button>
        <button class="btn-icon-danger" onclick="deleteExercise(${i})" aria-label="Удалить">🗑</button>
      </div>` : '';

    html += `
      <div class="card ${e.done ? 'done-row' : ''}" style="margin-bottom:12px;">
        <div class="card-header" style="display:flex; justify-content:space-between; align-items:center;">
          <span style="font-size:14px; font-weight:700; color:var(--text); text-transform:none;">${e.n}</span>
          <div class="exe-check ${e.done ? 'checked' : ''}" onclick="toggleExDone(${i})">${e.done ? '✓' : ''}</div>
        </div>
        <div class="card-body" style="padding:0">
          ${setsHtml}
        </div>
        ${editMode ? `<div style="padding:10px 14px">${editControls}</div>` : ''}
      </div>
    `;
  });

  el.innerHTML = html || '<div class="empty-state" style="text-align:center;padding:20px;">Нет упражнений</div>';

  if (editMode) {
    buttonsDiv.innerHTML = `<button onclick="openAddExerciseSheet()" style="width:100%; padding:14px; border-radius:var(--radius-md); background:var(--accent); color:#fff; border:none; font-weight:700; cursor:pointer;">+ Добавить упражнение</button>`;
  } else {
    const allDone = customWt.length > 0 && customWt.every(e => e.done);
    buttonsDiv.innerHTML = `
      <div class="execute-rest-box">
        <span>Таймер отдыха</span>
        <b id="rest-timer">01:30</b>
        <button class="confirm-btn-cancel" style="padding:6px 12px;min-height:36px" onclick="toggleRestTimer()">Старт</button>
      </div>
      <div style="display:flex; gap:8px; margin-top:12px;">
        <button onclick="cancelWorkout()" style="flex:1; padding:14px; border-radius:var(--radius-md); background:transparent; border:1px solid var(--danger); color:var(--danger); font-weight:700; cursor:pointer;">✕ Отмена</button>
        <button onclick="${allDone ? 'finishCustomWorkout()' : `toast('Выполни все подходы')`}" style="flex:2; padding:14px; border-radius:var(--radius-md); background:${allDone ? 'var(--success)' : 'var(--border)'}; color:${allDone ? '#fff' : 'var(--text-light)'}; border:none; font-weight:700; cursor:${allDone ? 'pointer' : 'default'};">✅ Завершить</button>
      </div>
    `;
  }
}

let restTimerId = null, restSeconds = 90;
function toggleRestTimer() {
  ...
  restTimerId = setInterval(() => {
    ...
    if (el) el.textContent = `${String(Math.floor(restSeconds / 60)).padStart(2, '0')}:${String(restSeconds % 60).padStart(2, '0')}`;
    if (!restSeconds) { clearInterval(restTimerId); restTimerId = null; toast('Время отдыха вышло!'); }
  }, 1000);
}
...
function confirmBackFromWorkout(){exitWorkoutExecution();}
window.confirmBackFromWorkout=confirmBackFromWorkout;

function cancelWorkout(){exitWorkoutExecution();}
window.cancelWorkout = cancelWorkout;

function deleteExercise(idx){
  const name = customWt[idx]?.n || 'упражнение';
  confirmDanger({
    title: 'Удалить упражнение?',
    message: `«${name}» и все его подходы будут удалены из тренировки.`,
    onConfirm: () => {
