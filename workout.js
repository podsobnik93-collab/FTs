...
// UPD: загрузка пользовательских программ из localStorage (интеграция с AI)
...
// Глобальная переменная ALL_EXERCISES (используется в других файлах)
window.ALL_EXERCISES = {};
for(const [cat, exs] of Object.entries(ENCYCLOPEDIA)){
  window.ALL_EXERCISES[cat] = exs.map(ex => ({...ex, s:3, r:12, kg:0, k:30, desc:'', tech:''}));
}
if(!window.ALL_EXERCISES.glutes) window.ALL_EXERCISES.glutes = [];
if(!window.ALL_EXERCISES.glutes.find(e=>e.n==='Ягодичный мостик со штангой')) window.ALL_EXERCISES.glutes.push({n:'Ягодичный мостик со штангой', s:3, r:15, kg:20, k:40, desc:'', tech:''});
if(!window.ALL_EXERCISES.back.find(e=>e.n==='Тяга горизонтального блока к поясу')) window.ALL_EXERCISES.back.push({n:'Тяга горизонтального блока к поясу', s:3, r:10, kg:50, k:45, desc:'', tech:''});
if(!window.ALL_EXERCISES.chest.find(e=>e.n==='Разводка гантелей лёжа')) window.ALL_EXERCISES.chest.push({n:'Разводка гантелей лёжа', s:3, r:10, kg:20, k:40, desc:'', tech:''});
if(!window.ALL_EXERCISES.legs.find(e=>e.n==='Жим платформы')) window.ALL_EXERCISES.legs.push({n:'Жим платформы', s:4, r:10, kg:80, k:60, desc:'', tech:''});

// ==================== ПРОГРАММЫ ТРЕНИРОВОК ====================
let PROGRAMS = [
  {
    "id":"womens",
    "name":"Женская тренировка",
    "level":"Средний",
    "days_week":"4 дня/нед",
    "goal":"Форма и тонус",
    "desc":"Акцент на ягодицы, ноги и пресс. Подходит для девушек любого уровня.",
    "schedule":[
      {"label":"Пн","name":"Ягодицы + Ноги","rest":false,"exs":["Румынская тяга","Приседания со штангой","Отведение ноги в кроссовере","Выпады с гантелями","Сгибание ног лёжа","Планка"]},
      {"label":"Вт","name":"Отдых","rest":true,"exs":[]},
      {"label":"Ср","name":"Спина + Плечи","rest":false,"exs":["Тяга верхнего блока широким прямым хватом к груди","Тяга горизонтального блока к поясу","Жим гантелей сидя или стоя","Разведение рук с гантелями в стороны","Тяга штанги в наклоне к поясу"]},
      {"label":"Чт","name":"Отдых","rest":true,"exs":[]},
      {"label":"Пт","name":"Ягодицы + Пресс","rest":false,"exs":["Ягодичный мостик со штангой","Мёртвая тяга на прямых ногах","Гиперэкстензия","Скручивания лёжа на наклонной скамье","Боковая планка","Подъём ног в висе"]},
      {"label":"Сб","name":"Грудь + Руки","rest":false,"exs":["Жим гантелей лёжа","Разводка гантелей лёжа","Сгибание рук в кроссовере","Жим лёжа узким хватом","Отжимания на брусьях"]},
      {"label":"Вс","name":"Отдых","rest":true,"exs":[]}
    ]
  },
  {
    "id":"split_back_bi",
    "name":"Сплит: Спина + Бицепс + Пресс",
    "level":"Средний",
    "days_week":"1 день/нед",
    "goal":"Масса и сила",
    "desc":"Классический сплит-день на спину и бицепс с проработкой пресса в конце.",
    "schedule":[
      {"label":"Пн","name":"Спина + Бицепс + Пресс","rest":false,"exs":["Подтягивания широким прямым хватом к груди","Тяга штанги в наклоне к поясу","Тяга горизонтального блока к поясу","Тяга гантели в наклоне одной рукой","Сгибание рук со штангой стоя","Сгибание рук с гантелями поочерёдно стоя","Планка","Скручивания лёжа на наклонной скамье"]},
      {"label":"Вт","name":"Отдых","rest":true,"exs":[]},
      {"label":"Ср","name":"Отдых","rest":true,"exs":[]},
      {"label":"Чт","name":"Отдых","rest":true,"exs":[]},
      {"label":"Пт","name":"Отдых","rest":true,"exs":[]},
      {"label":"Сб","name":"Отдых","rest":true,"exs":[]},
      {"label":"Вс","name":"Отдых","rest":true,"exs":[]}
    ]
  }
];

// Загрузка пользовательских программ (например, от ИИ-тренера)
(function loadCustomPrograms() {
  try {
    const saved = localStorage.getItem('fs-programs');
    if (saved) {
      const userProgs = JSON.parse(saved);
      userProgs.forEach(p => {
        if (!PROGRAMS.some(ex => ex.id === p.id)) {
          PROGRAMS.push(p);
        }
      });
    }
  } catch(e) { console.warn('Failed to load custom programs', e); }
})();

// ==================== ГЛОБАЛЬНЫЕ ПЕРЕМЕННЫЕ СОСТОЯНИЯ ====================
let customWt = [];            // текущая тренировка (массив упражнений)
let currentWtScreen = 'home'; // home, builder, execute
let editMode = false;
let pickerChecked = {};       // выбранные упражнения в конструкторе
let pickerParams = {};        // параметры (подходы, повторы, вес) для каждого упражнения
...
function saveWorkoutFlowState() {
  const state = {
    ...
    pickerChecked: Object.fromEntries(Object.entries(pickerChecked).map(([k,v]) => [k, Array.from(v)])),
    pickerParams,
    activeMuscle
  };
...
// Предупреждение при закрытии страницы с активной тренировкой
...
// ==================== ПРОГРАММЫ (отображение, выбор, старт) ====================
...
window.getPlannedWorkoutsForMonth = getPlannedWorkoutsForMonth;
...
function togglePrograms() {
  ...
  const arrow = document.getElementById('prog-toggle-arrow');
  if (!sec) return;
...
function renderProgList() {
  ...
  if (activeProgId) {
    const ap = PROGRAMS.find(p => p.id === activeProgId);
    if (ap && banner) {
      banner.style.display = 'block';
      banner.innerHTML = `✅ Активна: <strong>${ap.name}</strong> — ${ap.days_week}`;
    }
  } else if (banner) banner.style.display = 'none';
  el.innerHTML = PROGRAMS.map(p => `
    <div class="prog-card-item ${p.id === activeProgId ? 'active-prog' : ''}" onclick="viewProgram('${p.id}')">
      <div class="prog-card-top">
        <div class="prog-card-name">${p.name}</div>
        ${p.id === activeProgId ? '<span style="font-size:11px;color:var(--accent);font-weight:600">✅ Активна</span>' : ''}
      </div>
      <div class="prog-card-meta">
        <span class="prog-meta-tag">${p.level}</span>
        <span class="prog-meta-tag">📅 ${p.days_week}</span>
        <span class="prog-meta-tag">🎯 ${p.goal}</span>
      </div>
...
function viewProgram(id) {
  ...
  const cells = prog.schedule.map((d, i) => {
    const isToday = i === todayIdx;
    const icon = d.rest ? '🛌' : '🏋️';
    return `<div class="prog-day-cell ${d.rest ? 'rest' : ''} ${isToday ? 'today-cell' : ''}" onclick="${d.rest ? '' : `showProgDay('${id}',${i})`}">
      <div class="prog-day-label">${d.label}</div>
      <div class="prog-day-icon">${icon}</div>
      <div class="prog-day-name">${d.name.split('—')[0].trim().substring(0,10)}</div>
    </div>`;
  }).join('');
  const todayDay = prog.schedule[todayIdx];
  const todayBlock = todayDay && !todayDay.rest ? `
    <div style="background:color-mix(in srgb,var(--accent) 10%,transparent);border:1.5px solid var(--accent);border-radius:12px;padding:12px 14px;margin-bottom:12px">
      <div style="font-size:11px;font-weight:700;color:var(--accent);margin-bottom:4px">📅 Сегодня</div>
      <div style="font-weight:600;font-size:14px;margin-bottom:8px">${todayDay.name}</div>
      <div>${todayDay.exs.map(e => `<span class="prog-ex-chip">${e}</span>`).join('')}</div>
      <button class="prog-start-btn" onclick="startProgDay('${id}',${todayIdx})">▶ Начать тренировку сейчас</button>
    </div>
  ` : `<div style="text-align:center;padding:12px;color:var(--text-muted);font-size:13px;margin-bottom:12px">🛌 Сегодня день отдыха</div>`;
  const isActive = activeProgId === id;
  detailEl.innerHTML = `
    <div class="prog-back-btn" onclick="renderProgList()">← Все программы</div>
...
function showProgDay(progId, dayIdx) {
  ...
  if (!prog) return;
  const day = prog.schedule[dayIdx];
  if (!day || day.rest) return;
  const el = document.getElementById('prog-day-expanded');
  if (!el) return;
  el.innerHTML = `
    <div class="prog-day-detail">
      <div class="prog-day-detail-name">${day.label}: ${day.name}</div>
      <div style="margin-bottom:10px">${day.exs.map(e => `<span class="prog-ex-chip">${e}</span>`).join('')}</div>
      <button class="prog-start-btn" onclick="startProgDay('${progId}',${dayIdx})">▶ Начать эту тренировку</button>
    </div>
  `;
...
function startProgDay(progId, dayIdx) {
  ...
  const day = prog.schedule[dayIdx];
  if (!day || day.rest) return;
  const allEx = Object.values(window.ALL_EXERCISES).flat();
  customWt = day.exs.map(name => {
    const found = allEx.find(e => e.n === name);
    if (found) {
      return {
        ...found,
        sets_data: Array.from({length: found.s}, () => ({r: found.r, kg: found.kg, done: false})),
        done: false
      };
    }
    return {
      n: name, s: 3, r: 12, k: 30, kg: 0, m: '',
      sets_data: [{r:12,kg:0,done:false},{r:12,kg:0,done:false},{r:12,kg:0,done:false}],
      done: false
    };
  });
...
function showWtScreen(screen) {
  currentWtScreen = screen;
  saveWorkoutFlowState();
  if (typeof updateCtaLabel === 'function') updateCtaLabel();
  ...
  if (hdr) hdr.textContent = titles[screen] || 'FitSim';
...
function renderSavedWts() {
  const card = document.getElementById('saved-wt-card');
  const list = document.getElementById('saved-wt-list');
  const cnt = document.getElementById('saved-wt-count');
  ...
  if (cnt) cnt.textContent = savedWts.length + ' шаблонов';
  if (list) {
    list.innerHTML = savedWts.map((wt, i) => `
      <div class="saved-wt-item">
        <div class="saved-wt-info">
          <div class="saved-wt-name">${wt.name || 'Тренировка ' + (i+1)}</div>
          <div class="saved-wt-meta">${wt.exercises.length} упражнений</div>
        </div>
        <div class="saved-wt-btns">
          <button class="saved-wt-start" onclick="loadSavedWt(${i})">▶ Начать</button>
          <button class="saved-wt-del" onclick="deleteSavedWt(${i})">✕</button>
        </div>
      </div>
    `).join('');
  }
...
function loadSavedWt(idx) {
  const wt = savedWts[idx];
  if (!wt) return;
  customWt = wt.exercises.map(e => ({
    ...e,
    sets_data: new Array(e.s).fill(false).map(() => ({ r: e.r, kg: e.kg, done: false })),
    done: false
  }));
  showWtScreen('execute');
...
function deleteSavedWt(idx) {
  if (!confirm('Удалить сохранённую тренировку?')) return;
...
function initBuilder() {
  ...
  const ni = document.getElementById('wt-name-input');
...
function renderMuscleGrid() {
  ...
  if (!grid) return;
  const muscles = Object.keys(window.ALL_EXERCISES);
  ...
  grid.innerHTML = muscles.map(cat => `
    <div class="muscle-tile ${activeMuscle === cat ? 'active-muscle' : ''}" data-cat="${cat}" onclick="selectMuscle('${cat}')">
      <div>${icons[cat] || '💪'}</div>
      <div>${names[cat] || cat}</div>
    </div>
  `).join('');
}
...
function renderExPicker(cat) {
  ...
  const title = document.getElementById('ex-picker-title');
  ...
  const exs = window.ALL_EXERCISES[cat] || [];
  const checked = pickerChecked[cat] || new Set();
  if (!pickerParams[cat]) pickerParams[cat] = {};
  let html = '';
  for (let i = 0; i < exs.length; i++) {
    const e = exs[i];
    const isChk = checked.has(e.n);
    if (!pickerParams[cat][e.n]) pickerParams[cat][e.n] = { s: e.s, r: e.r, kg: e.kg };
    const p = pickerParams[cat][e.n];
    const paramsHtml = isChk ? `
      <div class="pick-params" onclick="event.stopPropagation()">
        <div class="pick-param-group">
          <div class="pick-param-lbl">Подходы</div>
          <div class="pick-param-ctrl">
            <button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','s',-1,${i})">−</button>
            <span class="pick-param-val" id="pp-s-${cat}-${i}">${p.s}</span>
            <button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','s',1,${i})">+</button>
          </div>
        </div>
        <div class="pick-param-group">
          <div class="pick-param-lbl">Повторы</div>
          <div class="pick-param-ctrl">
            <button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','r',-1,${i})">−</button>
            <span class="pick-param-val" id="pp-r-${cat}-${i}">${p.r}</span>
            <button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','r',1,${i})">+</button>
          </div>
        </div>
        <div class="pick-param-group">
          <div class="pick-param-lbl">Вес, кг</div>
          <div class="pick-param-ctrl">
            <button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','kg',-0.5,${i})">−</button>
            <input class="pick-param-input" id="pp-kg-${cat}-${i}" type="number" inputmode="decimal" value="${p.kg}" onfocus="this.select()" onchange="setPickParam('${cat}','${e.n}','kg',parseFloat(this.value)||0,${i})">
            <button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','kg',0.5,${i})">+</button>
          </div>
        </div>
      </div>
    ` : '';
    html += `
      <div class="ex-pick-row ${isChk ? 'row-checked' : ''}" data-i="${i}" data-cat="${cat}">
        <div class="ex-pick-row-top" onclick="togglePickEx(${i}, '${cat}')">
          <div class="ex-pick-check ${isChk ? 'checked' : ''}">${isChk ? '✓' : ''}</div>
          <div class="ex-pick-info"><div class="ex-pick-name">${e.n}</div></div>
        </div>
        ${paramsHtml}
      </div>
    `;
...
window.renderExPicker = renderExPicker;

function togglePickEx(exIdx, cat) {
  const e = (window.ALL_EXERCISES[cat] || [])[exIdx];
  if (!e) return;
  if (!pickerChecked[cat]) pickerChecked[cat] = new Set();
...
function adjPickParam(cat, name, field, delta, idx) {
  ...
  if (el) {
    if (el.tagName === 'INPUT') el.value = pickerParams[cat][name][field];
    else el.textContent = pickerParams[cat][name][field];
  }
}
...
function selectAllExs() {
  ...
  const exs = window.ALL_EXERCISES[activeMuscle] || [];
...
function updateBldStartBar() {
  const bar = document.getElementById('bld-start-bar');
  const label = document.getElementById('bld-start-label');
  let total = 0;
  for (const s of Object.values(pickerChecked)) total += s.size;
  if (bar) bar.style.display = total > 0 ? 'block' : 'none';
  if (label) label.textContent = '▶ Начать тренировку · ' + total + ' упр.';
}
window.updateBldStartBar = updateBldStartBar;

function startCustomWorkout() {
  customWt = [];
  for (const [cat, names] of Object.entries(pickerChecked)) {
    for (const name of names) {
      const e = (window.ALL_EXERCISES[cat] || []).find(x => x.n === name);
      if (e) {
        const p = (pickerParams[cat] && pickerParams[cat][name]) || { s: e.s, r: e.r, kg: e.kg };
        customWt.push({
          ...e,
          s: p.s, r: p.r, kg: p.kg,
          sets_data: Array.from({length: p.s}, () => ({ r: p.r, kg: p.kg, done: false })),
          done: false
        });
      }
    }
  ...
  const nameMap = { chest:'Грудь', back:'Спина', legs:'Ноги', shoulders:'Плечи', biceps:'Бицепс', triceps:'Трицепс', glutes:'Ягодицы', abs:'Пресс', forearms:'Предплечья', cardio:'Кардио' };
  const name = rawName || (Object.keys(pickerChecked).map(cat => nameMap[cat] || cat).join(' + ') || 'Тренировка');
...
function renderExecute() {
  updateExeProgress();
  const el = document.getElementById('exe-list');
  const buttonsDiv = document.getElementById('execute-buttons');
  if (!el) return;
  let html = '';
  for (let i = 0; i < customWt.length; i++) {
    const e = customWt[i];
    if (!e.sets_data) e.sets_data = Array.from({length: e.s}, (_, idx) => ({ r: e.r, kg: e.kg, done: false }));
    const setsHtml = e.sets_data.map((set, si) => `
      <div class="exe-set-row ${set.done ? 'done-set' : ''}">
        <div class="set-chip" onclick="toggleSet(${i},${si})">
          <div class="chk">✓</div>Подход ${si+1}
        </div>
        <div class="set-controls">
          <div class="set-ctrl">
            <button onclick="updateSetVal(${i},${si},'kg',-0.5)">-</button>
            <input class="set-kg-input" id="set-kg-${i}-${si}" type="number" inputmode="decimal" value="${set.kg}" onfocus="this.select()" onchange="setSetVal(${i},${si},'kg',parseFloat(this.value)||0)">
            кг
            <button onclick="updateSetVal(${i},${si},'kg',0.5)">+</button>
          </div>
          <div class="set-ctrl">
            <button onclick="updateSetVal(${i},${si},'r',-1)">-</button>
            <span id="set-r-${i}-${si}">${set.r} повт</span>
            <button onclick="updateSetVal(${i},${si},'r',1)">+</button>
          </div>
        </div>
      </div>
    ...
    const editControls = editMode ? `
      <div style="display:flex; gap:8px; margin-top:8px; justify-content:flex-end; border-top:1px solid var(--border); padding-top:8px;">
        <button onclick="editExercise(${i})" style="background:var(--input-bg); border:1px solid var(--border); border-radius:8px; padding:5px 10px; font-size:12px;">⚙️ Параметры</button>
        <button onclick="deleteExercise(${i})" style="background:#fdeaea; border:1px solid #e74c3c; border-radius:8px; padding:5px 10px; font-size:12px; color:#e74c3c;">✕ Удалить</button>
      </div>
    ` : '';
    html += `
      <div class="exe-ex-row ${e.done ? 'done-row' : ''}" id="exe-row-${i}">
        <div class="exe-ex-inner">
          <div class="exe-ex-left">
            <div class="exe-ex-name">${e.n}</div>
            <div class="exe-ex-detail" style="color:var(--text-muted); font-size:12px; margin-top:4px;">${e.s} подходов</div>
          </div>
          <div class="exe-check ${e.done ? 'checked' : ''}" onclick="toggleExDone(${i})">${e.done ? '✓' : ''}</div>
        </div>
        <div class="exe-sets-container" id="exe-sets-${i}">${setsHtml}</div>
        ${editControls}
      </div>
    `;
  }
  ...
  if (editMode) {
    buttonsDiv.innerHTML = `
      <button onclick="openAddExerciseSheet()" style="width:100%; padding:12px; border-radius:14px; background:var(--accent); color:#fff; font-weight:800; border:none; margin-bottom:8px;">➕ Добавить упражнение</button>
      <button onclick="toggleEditMode()" style="width:100%; padding:12px; border-radius:14px; background:var(--input-bg); color:var(--text); border:1px solid var(--border);">✅ Готово</button>
    `;
  } else {
    buttonsDiv.innerHTML = `
...
function updateExeProgress() {
  ...
  const bar = document.getElementById('exe-progress-bar');
  const txt = document.getElementById('exe-progress-text');
  if (bar) bar.style.width = pct + '%';
  if (txt) txt.textContent = `Выполнено: ${done} из ${total}`;
}
window.updateExeProgress = updateExeProgress;
...
window.generateWorkoutId = generateWorkoutId;
...
function finishCustomWorkout() {
  ...
  if (!done.length && !confirm('Ни один подход не отмечен. Всё равно сохранить?')) return;
  ...
  diary.unshift({
    ...
    date: today,
    type: name,
    exercises: toLog.map(e => {
      if (!e.sets_data) return `${e.n}: ${e.s}×${e.r}${e.kg > 0 ? ' - ' + e.kg + 'кг' : ''}`;
      const completedSets = e.sets_data.filter(s => s.done);
      const setsToLog = completedSets.length ? completedSets : e.sets_data;
      const groups = [];
      let current = null;
      for (const s of setsToLog) {
        if (!current || current.r !== s.r || current.kg !== s.kg) {
  ...
  toLog.forEach(e => {
    ...
    if (e.sets_data) {
      const activeSets = e.sets_data.filter(s => s.done).length ? e.sets_data.filter(s => s.done) : e.sets_data;
    ...
    exlog[e.n].push({ date: dateStr, kg: maxKg, s: totalSets, r: maxKgReps });
  });
...
function deleteExercise(idx) {
  if (confirm('Удалить упражнение "' + customWt[idx].n + '"?')) {
    customWt.splice(idx, 1);
    renderExecute();
  }
}
...
function saveExerciseParams() {
  ...
  if (newSets > oldSets) {
    for (let i = oldSets; i < newSets; i++) ex.sets_data.push({ r: newReps, kg: newKg, done: false });
  } else if (newSets < oldSets) {
    ex.sets_data = ex.sets_data.slice(0, newSets);
  } else {
    ex.sets_data.forEach(set => { set.r = newReps; set.kg = newKg; });
  }
  ex.done = false;
...
function renderAddMuscleGrid() {
  const grid = document.getElementById('add-muscle-grid');
  if (!grid) return;
  const muscles = Object.keys(window.ALL_EXERCISES);
  const icons = { chest:'💪', back:'🔙', legs:'🦵', shoulders:'🏋️', biceps:'💪', triceps:'🦾', glutes:'🍑', abs:'🎯', forearms:'✊', cardio:'🏃' };
  const names = { chest:'Грудные', back:'Спина', legs:'Ноги', shoulders:'Плечи', biceps:'Бицепс', triceps:'Трицепс', glutes:'Ягодицы', abs:'Пресс', forearms:'Предплечья', cardio:'Кардио' };
  grid.innerHTML = muscles.map(cat => `
    <div class="muscle-tile ${selectedAddCat === cat ? 'active-muscle' : ''}" onclick="selectAddMuscle('${cat}')">
      <div>${icons[cat] || '💪'}</div>
      <div>${names[cat] || cat}</div>
    </div>
  `).join('');
}
window.renderAddMuscleGrid = renderAddMuscleGrid;
...
function renderAddExList(cat) {
  ...
  const exs = window.ALL_EXERCISES[cat] || [];
  if (!exs.length) {
    container.innerHTML = '<div style="padding:16px; text-align:center; color:var(--text-muted)">Нет упражнений</div>';
    document.getElementById('add-ex-params').style.display = 'none';
    document.getElementById('confirm-add-ex-btn').style.display = 'none';
    return;
  }
  let html = '<div style="display:flex; flex-direction:column; gap:4px;">';
  exs.forEach((ex, idx) => {
    html += `
      <div class="ex-pick-row" data-idx="${idx}" onclick="selectAddExercise(${idx})" style="cursor:pointer; background:${selectedAddEx === ex ? 'var(--accent)' : 'transparent'}; color:${selectedAddEx === ex ? '#fff' : 'var(--text)'}; border-radius:8px; padding:8px 12px;">
  ...
  if (selectedAddEx) showAddExerciseParams(selectedAddEx);
  else {
    document.getElementById('add-ex-params').style.display = 'none';
    document.getElementById('confirm-add-ex-btn').style.display = 'none';
  }
...
function selectAddExercise(idx) {
  selectedAddEx = window.ALL_EXERCISES[selectedAddCat][idx];
...
function showAddExerciseParams(ex) {
  ...
  paramsDiv.innerHTML = `
    ...
    <div class="sheet-fields-row">
      <div class="sheet-field"><label>Подходы</label><input id="add-sets" type="number" min="1" value="${addExParams.s}"></div>
      <div class="sheet-field"><label>Повторы</label><input id="add-reps" type="number" min="1" value="${addExParams.r}"></div>
      <div class="sheet-field"><label>Вес (кг)</label><input id="add-kg" type="number" step="0.5" value="${addExParams.kg}"></div>
    </div>
  `;
  document.getElementById('confirm-add-ex-btn').style.display = 'block';
}
window.showAddExerciseParams = showAddExerciseParams;
