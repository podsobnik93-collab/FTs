...
function selectAddExercise(idx){
  ...
  if(searchVal) {
    // В режиме поиска — только подсвечиваем выбранное, не перерисовываем список
    document.querySelectorAll('#add-ex-list .ex-pick-row').forEach(row => {
      const nameEl = row.querySelector('.ex-pick-name');
      const isThis = nameEl && nameEl.textContent.trim() === selectedAddEx.n;
      row.style.background = isThis ? 'var(--accent)' : 'transparent';
      row.style.color = isThis ? '#fff' : 'var(--text)';
      const sub = row.querySelector('div:last-child');
      if(sub && sub !== nameEl) sub.style.color = isThis ? 'rgba(255,255,255,0.7)' : 'var(--text-muted)';
    });
  } else {
...
// Поиск по всем упражнениям (вызывается из шторки тренировки)
window.filterAddExSearch = function() {
  ...
  Object.entries(ALL_EXERCISES).forEach(([cat, exs]) => {
    exs.forEach((ex, idx) => { if (ex.n.toLowerCase().includes(q)) results.push({ex, cat, idx}); });
  });
  if (!results.length) {
  ...
  results.forEach(({ex, cat, idx}) => {
    ...
    html += `<div class="ex-pick-row" onclick="selectedAddCat='${cat}';selectAddExercise(${idx})" style="cursor:pointer;background:${isSel?'var(--accent)':'transparent'};color:${isSel?'#fff':'var(--text)'};border-radius:8px;padding:8px 12px;">
      <div class="ex-pick-name">${ex.n}</div>
      <div style="font-size:11px;color:${isSel?'rgba(255,255,255,0.7)':'var(--text-muted)'};margin-top:2px;">${catLabels[cat]||cat}</div>
    </div>`;
  });
  html += '</div>';
