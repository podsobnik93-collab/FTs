...
function openAddExerciseSheet(){
  ...
  const srch=document.getElementById('add-ex-search'); if(srch){ srch.value=''; srch.style.display='none'; }
  ...
  if(sheet) sheet.classList.add('show','add-ex-fullscreen');
  document.getElementById('sheet-overlay')?.classList.add('show');
}
function closeAddExerciseScreen(){
  const sheet=document.getElementById('add-ex-sheet');
  if(sheet) sheet.classList.remove('show','add-ex-fullscreen');
  document.getElementById('sheet-overlay')?.classList.remove('show');
}
window.closeAddExerciseScreen=closeAddExerciseScreen;

function renderAddMuscleGrid(){
  const grid=document.getElementById('add-muscle-grid'); if(!grid)return;
  const muscles=Object.keys(ALL_EXERCISES);
  const labels={chest:'Грудь',back:'Спина',legs:'Ноги',shoulders:'Плечи',biceps:'Бицепс',triceps:'Трицепс',glutes:'Ягодицы',abs:'Пресс',forearms:'Предплечья',cardio:'Кардио'};
  grid.innerHTML=`<div class="add-ex-screen-head"><button type="button" class="add-ex-back" onclick="closeAddExerciseScreen()" aria-label="Назад">‹</button><div><div class="add-ex-screen-title">Добавить упражнение</div><div class="add-ex-screen-subtitle">Выберите группу мышц</div></div></div><div class="add-ex-muscle-list">${muscles.map(m=>`<button type="button" class="add-ex-muscle-row" onclick="selectAddMuscle('${m}')"><span>${labels[m]||m}</span><i aria-hidden="true">›</i></button>`).join('')}</div>`;
}
function selectAddMuscle(cat){
  selectedAddCat=cat; selectedAddEx=null;
  const grid=document.getElementById('add-muscle-grid'); if(grid)grid.style.display='none';
  const search=document.getElementById('add-ex-search'); if(search){search.style.display='block';search.value='';search.placeholder='Поиск упражнений';}
  renderAddExList(cat);
}
function backToAddMuscles(){
  selectedAddCat=null; selectedAddEx=null;
  const list=document.getElementById('add-ex-list'); if(list)list.innerHTML='';
  const params=document.getElementById('add-ex-params'); if(params)params.style.display='none';
  const confirm=document.getElementById('confirm-add-ex-btn'); if(confirm)confirm.style.display='none';
  const search=document.getElementById('add-ex-search'); if(search){search.value='';search.style.display='none';}
  const grid=document.getElementById('add-muscle-grid'); if(grid)grid.style.display='';
  renderAddMuscleGrid();
}
window.backToAddMuscles=backToAddMuscles;

function renderAddExList(cat){
  const container=document.getElementById('add-ex-list'); const exs=ALL_EXERCISES[cat]||[];
  if(!container)return;
  const labels={chest:'Грудь',back:'Спина',legs:'Ноги',shoulders:'Плечи',biceps:'Бицепс',triceps:'Трицепс',glutes:'Ягодицы',abs:'Пресс',forearms:'Предплечья',cardio:'Кардио'};
  let html=`<div class="add-ex-screen-head add-ex-exercises-head"><button type="button" class="add-ex-back" onclick="backToAddMuscles()" aria-label="Назад">‹</button><div><div class="add-ex-screen-title">${labels[cat]||cat}</div><div class="add-ex-screen-subtitle">Выберите упражнение</div></div></div>`;
  if(!exs.length){container.innerHTML=html+'<div class="add-ex-empty">Упражнений нет</div>';return;}
  html+='<div class="add-exercise-list">';
  exs.forEach((ex,idx)=>{html+=`<button type="button" class="add-exercise-row${selectedAddEx===ex?' is-selected':''}" onclick="selectAddExercise(${idx})"><span>${ex.n}</span><i aria-hidden="true">›</i></button>`;});
  html+='</div>'; container.innerHTML=html;
  if(selectedAddEx)showAddExerciseParams(selectedAddEx);else{const p=document.getElementById('add-ex-params');if(p)p.style.display='none';const b=document.getElementById('confirm-add-ex-btn');if(b)b.style.display='none';}
}
...
function selectAddExerciseFromSearch(cat, idx) {
  ...
  selectedAddCat = cat;
  selectedAddEx = ex;
  addExParams = { s: ex.s || 3, r: ex.r || 12, kg: ex.kg || 0 };
  filterAddExSearch();
  showAddExerciseParams(ex);
  document.getElementById('add-ex-params')?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
window.selectAddExerciseFromSearch = selectAddExerciseFromSearch;

function closeAddExerciseParams() {
  document.getElementById('add-ex-params-modal')?.classList.remove('show');
}
window.closeAddExerciseParams = closeAddExerciseParams;
...
function showAddExerciseParams(ex) {
  ...
  document.getElementById('add-ex-params-modal')?.classList.add('show');
}
