...
function openAddExerciseSheet(){
  ...
  const exList=document.getElementById('add-ex-list'); if(exList) exList.innerHTML='';
  const exParams=document.getElementById('add-ex-params'); if(exParams) exParams.style.display='none';
  const confirmBtn=document.getElementById('confirm-add-ex-btn'); if(confirmBtn) confirmBtn.style.display='none';
...
function renderAddExList(cat){
  ...
  const allExs=ALL_EXERCISES[cat]||[];
  const exs=q ? allExs.filter(ex=>ex.n.toLowerCase().includes(q)) : allExs;
  if(!exs.length){container.innerHTML='<div style="padding:16px; text-align:center; color:var(--text-muted)">'+(q?'Ничего не найдено':'Нет упражнений')+'</div>'; document.getElementById('add-ex-params').style.display='none'; document.getElementById('confirm-add-ex-btn').style.display='none'; return;}
  let html='<div style="display:flex; flex-direction:column; gap:4px;">';
  exs.forEach((ex,idx)=>{const realIdx=allExs.indexOf(ex); html+=`<div class="ex-pick-row" data-idx="${realIdx}" onclick="selectAddExercise(${realIdx})" style="cursor:pointer; background:${selectedAddEx===ex?'var(--accent)':'transparent'}; color:${selectedAddEx===ex?'#fff':'var(--text)'}; border-radius:8px; padding:8px 12px;"><div class="ex-pick-name">${ex.n}</div></div>`;});
...
// Голосовой/текстовый поиск в шторке упражнений
window.filterAddExSearch = function(){
  const q=(document.getElementById('add-ex-search')?.value||'').toLowerCase().trim();
  if(!q){
    // Если категория выбрана — показываем список по категории
    if(selectedAddCat) renderAddExList(selectedAddCat);
    else { const l=document.getElementById('add-ex-list'); if(l) l.innerHTML=''; }
    return;
  }
  ...
  const results=[];
  Object.entries(ALL_EXERCISES).forEach(([cat,exs])=>{ exs.forEach(ex=>{ if(ex.n.toLowerCase().includes(q)) results.push({ex,cat}); }); });
  if(!results.length){ container.innerHTML='<div style="padding:16px; text-align:center; color:var(--text-muted)">Ничего не найдено</div>'; return; }
  let html='<div style="display:flex; flex-direction:column; gap:4px;">';
  results.forEach(({ex,cat})=>{
    const allExs=ALL_EXERCISES[cat]; const realIdx=allExs.indexOf(ex);
    ...
    html+=`<div class="ex-pick-row" onclick="selectedAddCat='${cat}';selectAddExercise(${realIdx})" style="cursor:pointer; border-radius:8px; padding:8px 12px; background:${selectedAddEx===ex?'var(--accent)':'transparent'}; color:${selectedAddEx===ex?'#fff':'var(--text)'};">
      <div class="ex-pick-name">${ex.n}</div>
      <div style="font-size:11px; color:var(--text-muted); margin-top:2px;">${catLabels[cat]||cat}</div>
    </div>`;
  });
  html+='</div>'; container.innerHTML=html;
};

// Восстановить сетку мышц при очистке поиска
document.addEventListener('DOMContentLoaded',()=>{
  const srch=document.getElementById('add-ex-search');
  if(srch){
    srch.addEventListener('input',()=>{
      const muscleGrid=document.getElementById('add-muscle-grid');
      if(muscleGrid) muscleGrid.style.display=srch.value.trim()?'none':'grid';
    });
  }
});

// Голосовой ввод в шторке добавления упражнения
let _sheetVoiceActive=false;
window.toggleVoiceInSheet=function(){
  const btn=document.getElementById('sheet-mic-btn');
  const input=document.getElementById('add-ex-search');
  if(!_sheetVoiceActive){
    ...
    _sheetVoiceActive=true;
    if(btn){ btn.style.background='#ef4444'; btn.style.animation='pulse 0.8s infinite'; }
    rec.onresult=e=>{
      const text=e.results[0][0].transcript;
      if(input){ input.value=text; window.filterAddExSearch(); }
      _sheetVoiceActive=false;
      if(btn){ btn.style.background='var(--accent)'; btn.style.animation=''; }
    };
    rec.onerror=()=>{ _sheetVoiceActive=false; if(btn){ btn.style.background='var(--accent)'; btn.style.animation=''; } };
    rec.onend=()=>{ _sheetVoiceActive=false; if(btn){ btn.style.background='var(--accent)'; btn.style.animation=''; } };
    rec.start();
  } else {
    _sheetVoiceActive=false;
    if(btn){ btn.style.background='var(--accent)'; btn.style.animation=''; }
