...
function showWtScreen(screen){
  ...
  const titles={home:'Тренировка',builder:'Создать тренировку',execute:'Выполнение'};
...
function renderMuscleGrid(){
  ...
  grid.innerHTML=muscles.map(m=>`<div class="muscle-tile${activeMuscle===m?' active-muscle':''}" data-cat="${m}" onclick="selectMuscle('${m}')" style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;padding:12px 8px;border-radius:12px;background:var(--input-bg);border:1.5px solid ${activeMuscle===m?'var(--accent)':'var(--border)'};cursor:pointer;min-height:64px;position:relative;">${counts[m]>0?`<span style="position:absolute;top:6px;right:6px;background:var(--accent);color:#fff;border-radius:9999px;font-size:10px;font-weight:700;min-width:18px;height:18px;display:flex;align-items:center;justify-content:center;padding:0 4px;">${counts[m]}</span>`:''}<span style="font-size:11px;font-weight:600;color:var(--text);text-align:center;">${labels[m]||m}</span></div>`).join('');
}
...
function selectMuscle(cat){
  ...
  const builder=document.getElementById('wt-builder');
  if(builder) builder.classList.add('has-muscle-picker');
  renderExPicker(cat);
  const picker=document.getElementById('ex-picker');
  if(picker){ picker.style.display='block'; picker.scrollIntoView({behavior:'smooth',block:'start'}); }
}
...
function backToMuscleGroups(){
  activeMuscle=null;
  const builder=document.getElementById('wt-builder');
  if(builder) builder.classList.remove('has-muscle-picker');
  const picker=document.getElementById('ex-picker');
  if(picker) picker.style.display='none';
  renderMuscleGrid();
}
window.backToMuscleGroups=backToMuscleGroups;

function getLastExerciseTemplate(name, fallback){
  const history=Array.isArray(exlog?.[name])?exlog[name]:[], last=history.at(-1), raw=last?.setsData||last?.sets_data;
  const setsData=Array.isArray(raw)&&raw.length?raw.map(x=>({r:Math.max(1,Number(x.r)||1),kg:Math.max(0,Number(x.kg)||0),done:false})):null;
  return {s:setsData?setsData.length:Math.max(1,Number(last?.s)||Number(fallback.s)||3),r:setsData?setsData[0].r:Math.max(1,Number(last?.r)||Number(fallback.r)||12),kg:setsData?setsData[0].kg:Math.max(0,Number(last?.kg)||Number(fallback.kg)||0),lastSetsData:setsData};
}

function getExerciseImage(name){
  return window.EXERCISE_IMAGES?.[name] || '';
}

function renderBuilderMuscleBack(){
  const picker=document.getElementById('ex-picker');
  if(!picker) return;
  let btn=document.getElementById('builder-muscles-back');
  if(!btn){
    btn=document.createElement('button');
    btn.id='builder-muscles-back';
    btn.type='button';
    btn.className='builder-muscles-back';
    btn.textContent='‹ Назад';
    btn.onclick=backToMuscleGroups;
    picker.insertBefore(btn,picker.firstChild);
  }
}
function renderExPicker(cat){
  renderBuilderMuscleBack();
  ...
  for(let i=0;i<exs.length;i++){
    ...
    if(!pickerParams[cat][e.n]) pickerParams[cat][e.n]=getLastExerciseTemplate(e.n,e);
    const p=pickerParams[cat][e.n];
    const paramsHtml=isChk?`<div class="pick-params" onclick="event.stopPropagation()"><div class="pick-param-group"><div class="pick-param-lbl">Подходы</div><div class="pick-param-ctrl"><button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','s',-1,${i})">−</button><input class="pick-param-input pick-param-number-input" id="pp-s-${cat}-${i}" type="number" min="1" max="10" value="${p.s}" inputmode="numeric" onchange="setPickParam('${cat}','${e.n}','s',parseInt(this.value,10),${i})" onfocus="this.select()"><button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','s',1,${i})">+</button></div></div><div class="pick-param-group"><div class="pick-param-lbl">Повторы</div><div class="pick-param-ctrl"><button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','r',-1,${i})">−</button><input class="pick-param-input pick-param-number-input" id="pp-r-${cat}-${i}" type="number" min="1" max="100" value="${p.r}" inputmode="numeric" onchange="setPickParam('${cat}','${e.n}','r',parseInt(this.value,10),${i})" onfocus="this.select()"><button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','r',1,${i})">+</button></div></div><div class="pick-param-group"><div class="pick-param-lbl">Вес, кг</div><div class="pick-param-ctrl"><button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','kg',-0.5,${i})">−</button><input class="pick-param-input" id="pp-kg-${cat}-${i}" type="number" inputmode="decimal" value="${p.kg}" onfocus="this.select()" onchange="setPickParam('${cat}','${e.n}','kg',parseFloat(this.value)||0,${i})"><button class="pick-param-btn" onclick="adjPickParam('${cat}','${e.n}','kg',0.5,${i})">+</button></div></div></div>`:'';
    html+=`<div class="ex-pick-row ${isChk?'row-checked':''}" data-i="${i}" data-cat="${cat}"><div class="ex-pick-row-top" onclick="togglePickEx(${i}, '${cat}')"><div class="ex-pick-check ${isChk?'checked':''}">${isChk?'✓':''}</div>${getExerciseImage(e.n)?`<img class="exercise-thumb" src="${getExerciseImage(e.n)}" alt="" loading="lazy">`:''}<div class="ex-pick-info"><div class="ex-pick-name">${e.n}</div></div></div>${paramsHtml}</div>`;
  }
...
function togglePickEx(exIdx,cat){
  ...
  else{checked.add(e.n); if(!pickerParams[cat])pickerParams[cat]={}; if(!pickerParams[cat][e.n]) pickerParams[cat][e.n]=getLastExerciseTemplate(e.n,e);}
  renderExPicker(cat);
}
...
function adjPickParam(cat,name,field,delta,idx){
  ...
  pickerParams[cat][name][field]=Math.max(mins[field],Math.min(maxs[field],cur+delta)); delete pickerParams[cat][name].lastSetsData;
...
function setPickParam(cat,name,field,val,idx){
  ...
  pickerParams[cat][name][field]=val; delete pickerParams[cat][name].lastSetsData;
  const el=document.getElementById(`pp-${field}-${cat}-${idx}`);
...
function getWorkoutStartWarning(){
  const profile=JSON.parse(localStorage.getItem('fs-profile')||'{}');
  const beginner=Number(profile.exp||0)<1;
  const selected=[];
  Object.entries(pickerChecked||{}).forEach(([cat,names])=>{
    Array.from(names||[]).forEach(name=>selected.push({cat,name,sets:Math.max(1,Number(pickerParams?.[cat]?.[name]?.s||3))}));
  });
  if(!selected.length) return '';
  const sets=selected.reduce((sum,x)=>sum+x.sets,0);
  const hard=/станов|присед.*штанг|жим.*штанг.*стоя|рывок|толчок|good morning/i;
  const hardCount=selected.filter(x=>hard.test(x.name)).length;
  if(beginner && hardCount>1) return 'В плане несколько сложных упражнений со свободным весом. Для новичка безопаснее начать с одного.';
  if(beginner && (selected.length>6 || sets>18)) return `В тренировке ${selected.length} упражнений и ${sets} подходов. Для начала это может быть многовато.`;
  if(beginner && selected.length<4) return 'В тренировке мало упражнений. Добавьте ещё 1–2, чтобы нагрузка была более полноценной.';
  return '';
}

function updateBldStartBar(){
  const bar=document.getElementById('bld-start-bar'),label=document.getElementById('bld-start-label'),outerBtn=bar?bar.querySelector('button'):null,builder=document.getElementById('wt-builder');
  let total=0;for(const s of Object.values(pickerChecked))total+=s.size;
  if(builder)builder.classList.toggle('has-selection',total>0);
  if(bar)bar.style.display=total>0?'block':'none';
  if(outerBtn)outerBtn.setAttribute('onclick','startCustomWorkout()');
  if(label)label.textContent='▶ Начать тренировку · '+total+' упр.';
}
function getWorkoutCategoriesFromExercises(exercises){
  const labels={chest:'Грудь',back:'Спина',legs:'Ноги',shoulders:'Плечи',biceps:'Бицепс',triceps:'Трицепс',glutes:'Ягодицы',abs:'Пресс',forearms:'Предплечья',cardio:'Кардио'};
  const used=[];
  (exercises||[]).forEach(ex=>{
    const cat=Object.keys(ALL_EXERCISES).find(key=>(ALL_EXERCISES[key]||[]).some(item=>item.n===ex.n));
    if(cat&&!used.includes(cat)) used.push(cat);
  });
  return used.map(cat=>labels[cat]||cat).join(' + ')||'Тренировка';
}
function syncWorkoutTitle(){
  const title=document.getElementById('exe-title');
  if(title) title.textContent=getWorkoutCategoriesFromExercises(customWt);
}

function startCustomWorkout(){
  const warning=getWorkoutStartWarning();
  if(warning && !confirm(`${warning}

Нажмите «ОК», чтобы начать тренировку, или «Отмена», чтобы вернуться к редактированию.`)) return;
  customWt=[];
  ...
  const activeCats=Object.keys(pickerChecked).filter(cat=>pickerChecked[cat]&&pickerChecked[cat].size>0);
  const name=rawName||(activeCats.map(cat=>({chest:'Грудь',back:'Спина',legs:'Ноги',shoulders:'Плечи',biceps:'Бицепс',triceps:'Трицепс',glutes:'Ягодицы',abs:'Пресс',forearms:'Предплечья',cardio:'Кардио'}[cat]||cat)).join(' + ')||'Тренировка');
  document.getElementById('exe-title').textContent=rawName||getWorkoutCategoriesFromExercises(customWt);
  showWtScreen('execute');
}
...
function toggleEditMode(){
  ...
  if(btn){
    btn.classList.toggle('is-editing',editMode);
    btn.setAttribute('aria-pressed',String(editMode));
    btn.setAttribute('aria-label',editMode?'Закончить редактирование':'Редактировать тренировку');
    btn.innerHTML=editMode?'✓ <span>Готово</span>':'✏️ <span>Редактировать</span>';
  }
}
...
function renderExecute(){
  ...
  if(editBtn){
    editBtn.classList.toggle('is-editing',editMode);
    editBtn.setAttribute('aria-pressed',String(editMode));
    editBtn.setAttribute('aria-label',editMode?'Закончить редактирование':'Редактировать тренировку');
    editBtn.innerHTML=editMode?'✓ <span>Готово</span>':'✏️ <span>Редактировать</span>';
  }
...
function addWorkoutSet(exIdx){
  const ex=customWt[exIdx];
  if(!ex)return;
  if(!Array.isArray(ex.sets_data))ex.sets_data=[];
  const lastSet=ex.sets_data[ex.sets_data.length-1]||{r:ex.r||12,kg:ex.kg||0};
  ex.sets_data.push({r:Math.max(1,Number(lastSet.r)||1),kg:Math.max(0,Number(lastSet.kg)||0),done:false});
  ex.s=ex.sets_data.length;
  ex.done=false;
  if(typeof saveWorkoutFlowState==='function')saveWorkoutFlowState();
  renderExecute();
}
window.addWorkoutSet=addWorkoutSet;

function removeWorkoutSet(exIdx){
  const ex=customWt[exIdx];
  if(!ex||!Array.isArray(ex.sets_data)||ex.sets_data.length<=1){toast('Нельзя удалить последний подход');return;}
  ex.sets_data.pop();
  ex.s=ex.sets_data.length;
  ex.done=ex.sets_data.every(set=>set.done);
  if(typeof saveWorkoutFlowState==='function')saveWorkoutFlowState();
  renderExecute();
}
window.removeWorkoutSet=removeWorkoutSet;
...
function finishCustomWorkout(){
  if(!customWt.length || !customWt.every(e => e.done)){
  ...
  let saved = false;
  try {
    const savedDiary = JSON.parse(localStorage.getItem('fs-diary') || '[]');
    const exlog = JSON.parse(localStorage.getItem('fs-exlog') || '{}');
    const name = getWorkoutCategoriesFromExercises(customWt);
    const title=document.getElementById('exe-title'); if(title) title.textContent=name;
    const date = new Date().toLocaleDateString('ru-RU');

    const exLines = customWt.map(e => {
      const groups = [];
      let current = null;
      const setsData = e.sets_data || e.setsdata || [];
    ...
    const kcal = Math.round(customWt.reduce((acc, e) => {
      const setsData = e.sets_data || e.setsdata || [];
      return acc + setsData.reduce((sum, set) => sum + ((Number(set.kg) || 0) * 0.05 * (Number(set.r) || 0)), 0);
    }, 0));

    savedDiary.unshift({
      id: typeof generateWorkoutId === 'function' ? generateWorkoutId() : `${Date.now()}-${Math.random().toString(36).slice(2)}`,
      date,
      type: name,
      exercises: exLines,
      kcal
    ...
    customWt.forEach(e => {
      if(!Array.isArray(exlog[e.n])) exlog[e.n] = [];
      const setsData = e.sets_data || e.setsdata || [];
      const maxSet = setsData.reduce((best, set) => Number(set.kg) > Number(best.kg) ? set : best, {kg: 0, r: 0});
      exlog[e.n].push({date,kg:Number(maxSet.kg)||0,s:setsData.length,r:Number(maxSet.r)||0,setsData:setsData.map(x=>({r:Number(x.r)||1,kg:Number(x.kg)||0}))});
    });

    localStorage.setItem('fs-diary', JSON.stringify(savedDiary));
    // Дневник в pages.js хранится также в памяти; синхронизируем его без перезагрузки.
    if(typeof diary !== 'undefined') diary = savedDiary;
    localStorage.setItem('fs-exlog', JSON.stringify(exlog));
    saved = true;
  } catch(error) {
    console.error('Ошибка сохранения тренировки:', error);
    toast('Не удалось полностью сохранить тренировку');
  } finally {
    customWt = [];
    currentWtScreen = 'home';
    editMode = false;
    if(typeof clearWorkoutFlowState === 'function') clearWorkoutFlowState();
    else localStorage.removeItem('fs-workout-flow-state');

    ['wt-execute', 'wt-builder'].forEach(id => {
      const el = document.getElementById(id);
      if(el) el.style.display = 'none';
    });
    const home = document.getElementById('wt-home');
    if(home) home.style.display = 'block';
    const bar = document.getElementById('bld-start-bar');
    if(bar) bar.style.display = 'none';
    ...
    toast(saved ? '✅ Тренировка сохранена!' : 'Тренировка завершена');
  }
}
window.finishCustomWorkout = finishCustomWorkout;

function exitWorkoutExecution(){
  customWt=[];
  editMode=false;
  pickerChecked={};
  pickerParams={};
...
window.exitWorkoutExecution=exitWorkoutExecution;
...
function cancelWorkout(){
  const shouldCancel=window.confirm('Отменить тренировку? Все незавершённые данные текущей тренировки будут потеряны.');
  if(!shouldCancel)return;
  exitWorkoutExecution();
}
window.cancelWorkout = cancelWorkout;

function deleteExercise(idx){
  if(confirm('Удалить упражнение "'+customWt[idx].n+'"?')){customWt.splice(idx,1); syncWorkoutTitle(); renderExecute();}
}
...
let addMode=false, addPickerChecked={}, addPickerParams={}, addActiveMuscle=null;

function openAddExerciseSheet(){
  const builder=document.getElementById('wt-builder');
  if(builder){
    builder.setAttribute('data-mode','add');
    const backBtn=builder.querySelector('.workout-builder-back');
    if(backBtn){backBtn.textContent='‹ К тренировке';backBtn.onclick=closeAddMode;}
  }

  addPickerChecked={};addPickerParams={};addActiveMuscle=null;
  // Switch to builder screen for adding exercises
  addPickerChecked={}; addPickerParams={}; addActiveMuscle=null;
  const wtExecute=document.getElementById('wt-execute');
  const wtBuilder=document.getElementById('wt-builder');
  const bldBar=document.getElementById('bld-start-bar');
  if(wtExecute) wtExecute.style.display='none';
  if(wtBuilder) wtBuilder.style.display='block';
  if(bldBar) bldBar.style.display='none';
  currentWtScreen='add';
  document.getElementById('header-title').textContent='Добавить упражнение';
  initAddBuilder();
}
window.openAddExerciseSheet = openAddExerciseSheet;

function initAddBuilder(){
  addActiveMuscle=null;
  const builder=document.getElementById('wt-builder');
  if(builder)builder.classList.remove('has-muscle-picker');
  document.querySelectorAll('.muscle-tile').forEach(t=>t.classList.remove('active-muscle'));
  const search=document.getElementById('builder-ex-search');if(search)search.value='';
  const searchList=document.getElementById('builder-ex-search-list');if(searchList){searchList.style.display='none';searchList.innerHTML='';}
  const picker=document.getElementById('ex-picker'),bar=document.getElementById('bld-start-bar');
  if(picker)picker.style.display='none';if(bar)bar.style.display='none';
  renderAddMuscleGrid();
}
...
function renderAddMuscleGrid(){
  ...
  const muscles=Object.keys(ALL_EXERCISES);
  const labels={chest:'Грудные',back:'Спина',legs:'Ноги',shoulders:'Плечи',biceps:'Бицепс',triceps:'Трицепс',glutes:'Ягодицы',abs:'Пресс',forearms:'Предплечья',cardio:'Кардио'};
  const existing=new Set(customWt.map(e=>e.n));
  grid.innerHTML=muscles.map(m=>{
    const available=(ALL_EXERCISES[m]||[]).some(e=>!existing.has(e.n));
    if(!available) return '';
    return `<div class="muscle-tile${addActiveMuscle===m?' active-muscle':''}" data-cat="${m}" onclick="selectAddMuscle('${m}')" style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;padding:12px 8px;border-radius:12px;background:var(--input-bg);border:1.5px solid ${addActiveMuscle===m?'var(--accent)':'var(--border)'};cursor:pointer;min-height:64px;"><span style="font-size:11px;font-weight:600;color:var(--text);text-align:center;">${labels[m]||m}</span></div>`;
  }).filter(Boolean).join('');
}
function selectAddMuscle(cat){
  addActiveMuscle=cat;
  const builder=document.getElementById('wt-builder');if(builder)builder.classList.add('has-muscle-picker');
  document.querySelectorAll('.muscle-tile').forEach(t=>t.classList.toggle('active-muscle',t.dataset&&t.dataset.cat===cat));
  renderAddExPicker(cat);
}

function renderAddMuscleBack(){
const picker=document.getElementById('ex-picker'),builder=document.getElementById('wt-builder');
if(!picker||!builder)return;
const obsolete=document.getElementById('add-muscles-back');if(obsolete)obsolete.remove();
const backBtn=builder.querySelector('.workout-builder-back');
if(backBtn){
backBtn.textContent='‹ К тренировке';
backBtn.onclick=closeAddMode;
if(backBtn.parentElement!==builder)builder.insertBefore(backBtn,builder.firstChild);
backBtn.style.display='inline-block';
}
let backInPicker=picker.querySelector('.builder-muscles-back');
if(!backInPicker){
backInPicker=document.createElement('button');
backInPicker.type='button';
backInPicker.className='builder-muscles-back';
picker.insertBefore(backInPicker,picker.firstChild);
}
backInPicker.textContent='‹ Назад';
backInPicker.onclick=backToAddMuscleGroups;
}
function backToAddMuscleGroups(){
  addActiveMuscle=null;const builder=document.getElementById('wt-builder');if(builder)builder.classList.remove('has-muscle-picker');
  const picker=document.getElementById('ex-picker');if(picker)picker.style.display='none';renderAddMuscleGrid();
}
window.backToAddMuscleGroups=backToAddMuscleGroups;

function renderAddExPicker(cat){
  renderAddMuscleBack();
  const picker=document.getElementById('ex-picker'),list=document.getElementById('ex-picker-list'),title=document.getElementById('ex-picker-title');
  if(!picker||!list)return;
  const LABELS={chest:'Грудь',back:'Спина',legs:'Ноги',shoulders:'Плечи',biceps:'Бицепс',triceps:'Трицепс',glutes:'Ягодицы',abs:'Пресс',forearms:'Предплечья',cardio:'Кардио'};
  if(title)title.textContent=(LABELS[cat]||cat)+' — выбери упражнения';
  const exs=ALL_EXERCISES[cat]||[],existing=new Set(customWt.map(e=>e.n)),available=exs.filter(e=>!existing.has(e.n));
  if(!addPickerChecked[cat])addPickerChecked[cat]=new Set();if(!addPickerParams[cat])addPickerParams[cat]={};const checked=addPickerChecked[cat];let html='';
  for(let i=0;i<available.length;i++){
    const e=available[i],isChk=checked.has(e.n);if(!addPickerParams[cat][e.n])addPickerParams[cat][e.n]={s:e.s||3,r:e.r||12,kg:e.kg||0};const p=addPickerParams[cat][e.n];
    const params=isChk?`<div class="pick-params" onclick="event.stopPropagation()"><div class="pick-param-group"><div class="pick-param-lbl">Подходы</div><div class="pick-param-ctrl"><button class="pick-param-btn" onclick="adjAddPickParam('${cat}','${e.n}','s',-1,${i})">−</button><input class="pick-param-input pick-param-number-input" id="pp-s-${cat}-${i}" type="number" min="1" max="10" value="${p.s}" onchange="setAddPickParam('${cat}','${e.n}','s',parseInt(this.value,10),${i})" onfocus="this.select()"><button class="pick-param-btn" onclick="adjAddPickParam('${cat}','${e.n}','s',1,${i})">+</button></div></div><div class="pick-param-group"><div class="pick-param-lbl">Повторы</div><div class="pick-param-ctrl"><button class="pick-param-btn" onclick="adjAddPickParam('${cat}','${e.n}','r',-1,${i})">−</button><input class="pick-param-input pick-param-number-input" id="pp-r-${cat}-${i}" type="number" min="1" max="100" value="${p.r}" onchange="setAddPickParam('${cat}','${e.n}','r',parseInt(this.value,10),${i})" onfocus="this.select()"><button class="pick-param-btn" onclick="adjAddPickParam('${cat}','${e.n}','r',1,${i})">+</button></div></div><div class="pick-param-group"><div class="pick-param-lbl">Вес, кг</div><div class="pick-param-ctrl"><button class="pick-param-btn" onclick="adjAddPickParam('${cat}','${e.n}','kg',-0.5,${i})">−</button><input class="pick-param-input" id="pp-kg-${cat}-${i}" type="number" value="${p.kg}" onchange="setAddPickParam('${cat}','${e.n}','kg',parseFloat(this.value)||0,${i})" onfocus="this.select()"><button class="pick-param-btn" onclick="adjAddPickParam('${cat}','${e.n}','kg',0.5,${i})">+</button></div></div></div>`:'';
    const imgSrc=typeof getExerciseImage==='function'?getExerciseImage(e.n):'';
    html+=`<div class="ex-pick-row${isChk?' row-checked':''}" data-i="${i}" data-cat="${cat}"><div class="ex-pick-row-top" onclick="toggleAddPickEx(${i},'${cat}')"><div class="ex-pick-check${isChk?' checked':''}">${isChk?'✓':''}</div>${imgSrc?`<img class="exercise-thumb" src="${imgSrc}" alt="" loading="lazy">`:''}<div class="ex-pick-info"><div class="ex-pick-name">${e.n}</div></div></div>${params}</div>`;
  }
  list.innerHTML=html||'<div style="padding:16px;color:var(--text-muted);text-align:center">Все упражнения этой группы уже добавлены</div>';picker.style.display='block';updateAddStartBar();
}

function toggleAddPickEx(exIdx, cat){
  const available=(ALL_EXERCISES[cat]||[]).filter(e=>!customWt.some(x=>x.n===e.n));
  const e=available[exIdx];
  if(!e) return;
  if(!addPickerChecked[cat]) addPickerChecked[cat]=new Set();
  const checked=addPickerChecked[cat];
  if(checked.has(e.n)) checked.delete(e.n); else checked.add(e.n);
  renderAddExPicker(cat);
}

function adjAddPickParam(cat,name,field,delta,idx){
  if(!addPickerParams[cat]||!addPickerParams[cat][name]) return;
  const mins={s:1,r:1,kg:0}, maxs={s:10,r:100,kg:500};
  const cur=addPickerParams[cat][name][field];
  addPickerParams[cat][name][field]=Math.max(mins[field], Math.min(maxs[field], cur+delta));
  if(field==='kg') addPickerParams[cat][name][field]=Math.round(addPickerParams[cat][name][field]*2)/2;
  renderAddExPicker(cat);
}

function setAddPickParam(cat,name,field,val,idx){
  if(!addPickerParams[cat]||!addPickerParams[cat][name]) return;
  const mins={s:1,r:1,kg:0}, maxs={s:10,r:100,kg:500};
  val=Math.max(mins[field], Math.min(maxs[field], val));
  if(field==='kg') val=Math.round(val*2)/2;
  addPickerParams[cat][name][field]=val;
}

function updateAddStartBar(){
  const bar=document.getElementById('bld-start-bar'),label=document.getElementById('bld-start-label'),outerBtn=bar?bar.querySelector('button'):null,builder=document.getElementById('wt-builder');
  let total=0;for(const s of Object.values(addPickerChecked))total+=s.size;
  if(builder)builder.classList.toggle('has-selection',total>0);
  if(bar)bar.style.display=total>0?'block':'none';
  if(outerBtn)outerBtn.setAttribute('onclick','confirmAddExercise()');
  if(label)label.textContent='▶ Добавить · '+total+' упр.';
}

function confirmAddExercise(){
  const additions=[];
  const existing=new Set(customWt.map(e=>e.n));
  for(const [cat,names] of Object.entries(addPickerChecked||{})){
    for(const name of names){
      if(existing.has(name))continue;
      const e=(ALL_EXERCISES[cat]||[]).find(x=>x.n===name);
      const p=addPickerParams?.[cat]?.[name]||{s:e?.s||3,r:e?.r||12,kg:e?.kg||0};
      if(!e)continue;
      const s=Math.max(1,parseInt(p.s,10)||3),r=Math.max(1,parseInt(p.r,10)||12),kg=Math.max(0,parseFloat(p.kg)||0);
      additions.push({...e,s,r,kg,sets_data:Array.from({length:s},()=>({r,kg,done:false})),done:false});
      existing.add(name);
    }
  }
  customWt.push(...additions);
  closeAddMode();
  syncWorkoutTitle();
  if(typeof saveWorkoutFlowState==='function')saveWorkoutFlowState();
  const list=document.getElementById('exe-list');
  if(list)renderExecute();
  else requestAnimationFrame(()=>renderExecute());
  toast(additions.length?`Добавлено ${additions.length} упр.`:'Новые упражнения не выбраны');
}

function closeAddMode(){
  const wtExecute=document.getElementById('wt-execute'),wtBuilder=document.getElementById('wt-builder'),picker=document.getElementById('ex-picker'),bar=document.getElementById('bld-start-bar'),outerBtn=bar?bar.querySelector('button'):null;
  if(wtExecute)wtExecute.style.display='block';if(wtBuilder){wtBuilder.style.display='none';wtBuilder.classList.remove('has-muscle-picker','has-selection');wtBuilder.removeAttribute('data-mode');const backBtn=wtBuilder.querySelector('.workout-builder-back');if(backBtn){backBtn.textContent='‹ Тренировка';backBtn.onclick=exitWorkoutBuilder;wtBuilder.insertBefore(backBtn,wtBuilder.firstChild);}}
  if(picker)picker.style.display='none';if(bar)bar.style.display='none';if(outerBtn)outerBtn.setAttribute('onclick','startCustomWorkout()');
  const inner=document.getElementById('add-muscles-back');if(inner)inner.remove();currentWtScreen='execute';const h=document.getElementById('header-title');if(h)h.textContent='Выполнение';addPickerChecked={};addPickerParams={};addActiveMuscle=null;
}
...
const MUSCLE_VOICE_MAP = {
  chest: ['грудь', 'грудные', 'грудная', 'грудных', 'грудную'],
  back: ['спина', 'спину', 'спине', 'спины'],
  legs: ['ноги', 'ногу', 'ноге', 'ног', 'нога'],
  shoulders: ['плечи', 'плечо', 'плечах', 'плеч', 'дельты'],
  biceps: ['бицепс', 'бицепсы', 'бицепса'],
  triceps: ['трицепс', 'трицепсы', 'трицепса'],
  glutes: ['ягодицы', 'ягодица', 'попа', 'ягодиц'],
  abs: ['пресс', 'живот', 'прессом', 'кубики'],
  forearms: ['предплечья', 'предплечье', 'предплечий'],
  cardio: ['кардио', 'бег', 'кардиотренировка', 'бегом']
};

function handleBuilderVoiceSearch() {
  const input = document.getElementById('builder-ex-search');
  if (!input) { searchBuilderEx(); return; }
  const raw = input.value.toLowerCase().trim();

  const hasDigitsOrParams = /\d|подход|повтор|килограмм|кг\b/i.test(raw);
  if (!hasDigitsOrParams) {
    for (const [cat, words] of Object.entries(MUSCLE_VOICE_MAP)) {
      if (words.some(w => raw.includes(w))) {
        ...
        selectMuscle(cat);
        const labels = {chest:'Грудь',back:'Спина',legs:'Ноги',shoulders:'Плечи',biceps:'Бицепс',triceps:'Трицепс',glutes:'Ягодицы',abs:'Пресс',forearms:'Предплечья',cardio:'Кардио'};
        toast('🎯 Открыта группа: ' + (labels[cat] || cat));
        return;
      }
    }
  }

  if (hasDigitsOrParams && typeof parseVoiceForBuilder === 'function' && typeof fuzzyMatchExercise === 'function') {
    const { exName, sets, reps, kg } = parseVoiceForBuilder(raw);
    if (exName && exName.length > 2) {
      ...
      if (found) {
        ...
        updateBldStartBar();
        if (typeof showVoiceResult === 'function') showVoiceResult(found.n + ' — ' + sets + '×' + reps + (kg ? ', ' + kg + ' кг' : ''));
        toast('✅ ' + found.n);
        return;
      }
    }
    if (activeMuscle && pickerChecked[activeMuscle] && pickerChecked[activeMuscle].size > 0) {
      const lastName = Array.from(pickerChecked[activeMuscle]).pop();
      if (!pickerParams[activeMuscle]) pickerParams[activeMuscle] = {};
      if (!pickerParams[activeMuscle][lastName]) pickerParams[activeMuscle][lastName] = { s: 3, r: 10, kg: 0 };
      if (sets) pickerParams[activeMuscle][lastName].s = sets;
      if (reps) pickerParams[activeMuscle][lastName].r = reps;
      if (kg) pickerParams[activeMuscle][lastName].kg = kg;
      renderExPicker(activeMuscle);
      updateBldStartBar();
      input.value = '';
      toast('🔧 Параметры обновлены: ' + lastName);
      return;
    }
  }
...
// Swipe-down close for the add-exercise sheet
(function attachAddExerciseSheetSwipe(){
  const sheet=document.getElementById('add-ex-sheet');
  if(!sheet||sheet.dataset.swipeCloseAttached)return;
  sheet.dataset.swipeCloseAttached='1';
...
/* FITSIM — в кнопке режима редактирования оставлена только иконка */
(function(){
  function updateEditIcon(){
    var btn=document.getElementById('edit-mode-btn');
    if(!btn)return;
    btn.innerHTML='<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>';
    btn.setAttribute('aria-label','Редактировать тренировку');
    btn.setAttribute('title','Редактировать тренировку');
  }
  var oldToggle=window.toggleEditMode;
  if(typeof oldToggle==='function')window.toggleEditMode=function(){oldToggle();updateEditIcon();};
  var oldRender=window.renderExecute;
  if(typeof oldRender==='function')window.renderExecute=function(){oldRender();updateEditIcon();};
  document.addEventListener('DOMContentLoaded',updateEditIcon);
  setTimeout(updateEditIcon,0);
})();
