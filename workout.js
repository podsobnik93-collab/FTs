...
function filterAddExSearch() {
  ...
  const qWords = raw.split(/\s+/).filter(w => w.length > 1);
    ...
    for (const w of qWords) {
      if (n.includes(w)) {
        ...
        for (const nw of nWords) {
          ...
          if (minLen >= 3 && (nw.startsWith(w.slice(0, minLen)) || w.startsWith(nw.slice(0, minLen)))) {
            score += 8;
            partial = true;
            break;
          }
        }
        if (!partial) allWordsMatch = false;
      }
    }
    if (!allWordsMatch && score < 15) return 0;
    return score;
  }
  let results = [];
  const catLabels = {
