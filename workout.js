...
function openAddExerciseSheet(){
  selectedAddCat=null; selectedAddEx=null; addExParams={s:3,r:12,kg:0}; searchResults=[];
  ...
  const ap=document.getElementById('add-ex-params'); if(ap){ap.style.display='none';ap.innerHTML='';}
  const cb=document.getElementById('confirm-add-ex-btn'); if(cb) cb.style.display='none';
  document.getElementById('add-ex-params-modal')?.classList.remove('show');
  renderAddMuscleGroups();
...
function renderAddMuscleGroups(){
  const container=document.getElementById('add-muscle-grid');
  if(!container) return;
  const muscles=Object.keys(ALL_EXERCISES);
  const labels={chest:'Грудь',back:'Спина',legs:'Ноги',shoulders:'Плечи',biceps:'Бицепс',triceps:'Трицепс',glutes:'Ягодицы',abs:'Пресс',forearms:'Предплечья',cardio:'Кардио'};
  const emojis={chest:'🔥',back:'🔙',legs:'🦵',shoulders:'🏋️',biceps:'💪',triceps:'💪',glutes:'🍑',abs:'⬡',forearms:'✊',cardio:'❤️'};
  container.innerHTML=`<button type="button" class="builder-muscles-back" onclick="closeAllSheets()" style="margin-bottom:12px">‹ Закрыть</button><h2 style="margin-bottom:16px">Добавить упражнение</h2><div style="margin-bottom:8px;font-size:13px;color:var(--text-muted)">Выберите группу мышц</div><div style="display:flex;flex-direction:column;gap:8px">`+
    muscles.map(m=>`<button type="button" onclick="selectAddMuscle('${m}')" style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;background:var(--input-bg);border:1.5px solid ${selectedAddCat===m?'var(--accent)':'var(--border)'};border-radius:12px;cursor:pointer;font-size:15px;font-weight:600;color:var(--text)"><span>${emojis[m]||''} ${labels[m]||m}</span><span style="font-size:18px">›</span></button>`).join('')+`</div>`;
}

function selectAddMuscle(cat){
  selectedAddCat=cat;
  renderAddExerciseList(cat);
}

function renderAddExerciseList(cat){
  const container=document.getElementById('add-muscle-grid');
  if(!container) return;
  const exs=ALL_EXERCISES[cat]||[];
  const labels={chest:'Грудь',back:'Спина',legs:'Ноги',shoulders:'Плечи',biceps:'Бицепс',triceps:'Трицепс',glutes:'Ягодицы',abs:'Пресс',forearms:'Предплечья',cardio:'Кардио'};
  const existing=new Set(customWt.map(e=>e.n));
  const available=exs.filter(e=>!existing.has(e.n));
  if(!available.length){
    container.innerHTML=`<button type="button" class="builder-muscles-back" onclick="renderAddMuscleGroups()">‹ Назад</button><h2 style="margin-bottom:16px">${labels[cat]||cat}</h2><div style="padding:24px;text-align:center;color:var(--text-muted)">Все упражнения этой группы уже добавлены</div>`;
    return;
  }
  container.innerHTML=`<button type="button" class="builder-muscles-back" onclick="renderAddMuscleGroups()">‹ Назад</button><h2 style="margin-bottom:8px">${labels[cat]||cat}</h2><div style="margin-bottom:16px;font-size:13px;color:var(--text-muted)">Отмеченные упражнения добавятся в текущую тренировку</div><div style="display:flex;flex-direction:column;gap:6px">`+
    available.map((e,i)=>`<label style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--card-inner);border:1px solid var(--border);border-radius:10px;cursor:pointer"><input type="checkbox" data-ex-name="${e.n.replace(/"/g,'&quot;')}" onchange="updateAddConfirmBtn()"><span style="font-size:14px;font-weight:500">${e.n}</span></label>`).join('')+`</div><div id="add-ex-confirm-wrap" style="margin-top:16px;display:none"><button id="confirm-add-ex-btn" type="button" style="width:100%;padding:14px;background:var(--accent);color:#fff;border:none;border-radius:14px;font-size:15px;font-weight:700;cursor:pointer" onclick="confirmAddExercise()"></button></div>`;
}

function updateAddConfirmBtn(){
  const cbs=document.querySelectorAll('#add-muscle-grid input[type=checkbox]:checked');
  const wrap=document.getElementById('add-ex-confirm-wrap');
  const btn=document.getElementById('confirm-add-ex-btn');
  if(!wrap||!btn) return;
  if(cbs.length>0){
    wrap.style.display='block';
    btn.textContent='Добавить · '+cbs.length+' упр.';
  }else{
...
function confirmAddExercise(){
  const cbs=document.querySelectorAll('#add-muscle-grid input[type=checkbox]:checked');
  if(!cbs.length){toast('Выберите хотя бы одно упражнение');return;}
  const labels={chest:'Грудь',back:'Спина',legs:'Ноги',shoulders:'Плечи',biceps:'Бицепс',triceps:'Трицепс',glutes:'Ягодицы',abs:'Пресс',forearms:'Предплечья',cardio:'Кардио'};
  const cat=selectedAddCat;
  cbs.forEach(cb=>{
    const name=cb.getAttribute('data-ex-name');
    const ex=(ALL_EXERCISES[cat]||[]).find(e=>e.n===name);
    if(ex){
      const p=addExParams||{s:3,r:12,kg:0};
      customWt.push({...ex,s:p.s,r:p.r,kg:p.kg,sets_data:Array.from({length:p.s},()=>({r:p.r,kg:p.kg,done:false})),done:false});
    }
  });
  syncWorkoutTitle();
  closeAllSheets();
  renderExecute();
  toast('Добавлено '+cbs.length+' упр.');
}
