...
function showStatsTip(key) {
  const p = profile || {};
  const expLabels = {'0':'новичок','0.5':'до 6 мес','1':'6–12 мес','2':'1–2 года','3':'2–5 лет','5':'5+ лет'};
  const coachLabels = {'no':'самостоятельно','past':'опыт с тренером','now':'с тренером сейчас'};
  const expLbl   = expLabels[String(window._expVal  !== undefined ? window._expVal  : 0)] || '';
  const coachLbl = coachLabels[window._coachVal || 'no'] || '';
  const delta    = window._expDelta || 0;
  const sign     = delta > 0 ? '+' : '';
  const factor   = window._expFactor || 1;

  const tips = {
    goal: {
      title: '⚖️ Твоя цель',
      body: 'Целевой вес, к которому ты стремишься. Влияет на автоматический выбор стратегии: похудение, поддержание или набор массы.'
    },
    bmi: {
      title: '📊 Индекс массы тела (ИМТ)',
      body: 'ИМТ = вес ÷ рост². Норма: 18.5–24.9. При наборе мышечной массы ИМТ 25–27 допустим — мышцы тяжелее жира, и это не ожирение.'
    },
    bmr: {
      title: '🛌 Базальный метаболизм (BMR)',
      body: 'Калории, которые тело сжигает в полном покое — на дыхание, сердце, органы. Рассчитывается по формуле Миффлина–Сан Жеора с поправкой на возраст.'
    },
    tdee: {
      title: '🔥 Реальный расход в день (TDEE)',
      body: 'BMR × коэффициент активности. Столько калорий ты тратишь в реальности с учётом тренировок и образа жизни.'
        + (factor !== 1.00
            ? '\n\n📌 Поправка стажа (' + expLbl + (coachLbl !== 'самостоятельно' ? ', ' + coachLbl : '') + '): ' + sign + delta + ' ккал\nОпытные атлеты сжигают больше из-за большей мышечной массы; новички — чуть меньше, пока тело адаптируется.'
            : '')
    },
    kcal: {
      title: '🎯 Рекомендуемые ккал в день',
      body: 'TDEE ± поправка на цель:\n• Похудение: −400 ккал (−300 после 55 лет)\n• Поддержание: без изменений\n• Набор массы: +300 ккал (+200 после 55 лет)\n\nМинимум — 1200 ккал, ниже нельзя.'
    }
  };
  ...
  const old = document.getElementById('stats-tip-overlay');
  if (old) old.remove();
  ...
  overlay.innerHTML =
    '<div style="background:var(--card-bg,#1c1c1e);border-radius:20px 20px 0 0;padding:24px 20px 36px;width:100%;max-width:480px;box-shadow:0 -4px 32px rgba(0,0,0,0.4);">' +
      '<div style="width:36px;height:4px;background:var(--border);border-radius:4px;margin:0 auto 18px;"></div>' +
      ...
      '<button onclick="document.getElementById(\'stats-tip-overlay\').remove()" style="margin-top:20px;width:100%;padding:12px;background:var(--input-bg);border:1px solid var(--border);border-radius:12px;color:var(--text);font-size:15px;font-weight:600;cursor:pointer;">Понятно</button>' +
    '</div>';
