...
function renderNutrEntries() {
  ...
  // FIX: анимируем три дуги БЖУ пропорционально вкладу каждого макронутриента
  // Обхват окружности r=72: 2 * π * 72 ≈ 452
  const _C = 452;
  const _setArc = (id, kcalPart) => {
    const el = document.getElementById(id);
    if (!el) return;
    const pct = Math.min(1, _kGoal > 0 ? kcalPart / _kGoal : 0);
    el.setAttribute('stroke-dasharray', (pct * _C).toFixed(1) + ' ' + _C);
  };
  _setArc('nutr-arc-prot', totals.protein * 4);
  _setArc('nutr-arc-fat',  totals.fat     * 9);
  _setArc('nutr-arc-carb', totals.carbs   * 4);
