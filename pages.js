...
// Body sheet (замеры тела)
(function(){
  var sh = document.getElementById('body-sheet');
  if(!sh || sh._swipeReady) return;
  sh._swipeReady = true;
  var sy=0, cy=0, drag=false;
  sh.addEventListener('touchstart', function(e){
    if(e.target.tagName==='BUTTON'||e.target.tagName==='INPUT') return;
    sy=e.touches[0].clientY; cy=sy; drag=true; sh.style.transition='none';
  },{passive:true});
  sh.addEventListener('touchmove', function(e){
    if(!drag) return; e.preventDefault(); cy=e.touches[0].clientY;
    var dy=Math.max(0,cy-sy); sh.style.transform='translateY('+dy+'px)';
  },{passive:false});
  sh.addEventListener('touchend', function(){
    if(!drag) return; drag=false; var dy=cy-sy;
    sh.style.transition='transform 0.25s cubic-bezier(0.32,0.72,0,1)';
    if(dy>80){closeAllSheets();}else{sh.style.transform='translateY(0)';}
  });
})();
...
(function(){
  ...
  sh.addEventListener('touchend', function(){
    if(!drag) return; drag=false; var dy=cy-sy;
    sh.style.transition='transform 0.25s cubic-bezier(0.32,0.72,0,1)';
    if(dy>80){closeCalendarNoteSheet();}else{sh.style.transform='translateY(0)';}
  });
})();
