...
function getCalendarNotes() {
  try { const notes = JSON.parse(localStorage.getItem('fs-calendar-notes') || '{}'); return notes && typeof notes === 'object' && !Array.isArray(notes) ? notes : {}; } catch (e) { return {}; }
}
function escapeCalendarNote(text) { const el=document.createElement('div'); el.textContent=String(text||''); return el.innerHTML; }
function noteDateToIso(dateStr) { const p=String(dateStr||'').split('.'); return p.length===3 ? `${p[2]}-${p[1]}-${p[0]}` : ''; }
function isoToNoteDate(iso) { const p=String(iso||'').split('-'); return p.length===3 ? `${p[2]}.${p[1]}.${p[0]}` : ''; }
function openCalendarNoteSheet(dateStr) {
  const selected=dateStr || window._calSelectedDate || isoToNoteDate(new Date().toISOString().slice(0,10));
  const notes=getCalendarNotes(), oldText=notes[selected] || '';
  let overlay=document.getElementById('calendar-note-overlay'), sheet=document.getElementById('calendar-note-sheet');
  if(!overlay){ overlay=document.createElement('div'); overlay.id='calendar-note-overlay'; overlay.className='sheet-overlay'; overlay.onclick=closeCalendarNoteSheet; document.body.appendChild(overlay); }
  if(!sheet){ sheet=document.createElement('div'); sheet.id='calendar-note-sheet'; sheet.className='bottom-sheet calendar-note-sheet'; document.body.appendChild(sheet); }
  sheet.innerHTML=`<div class="sheet-handle"></div><div class="sheet-title">Заметка в календаре</div><div class="sheet-field"><label>Дата</label><input id="cal-note-date" type="date" value="${noteDateToIso(selected)}"></div><div class="sheet-field"><label>Заметка</label><textarea id="cal-note-text" maxlength="500" placeholder="Например: замерить вес, купить протеин, день отдыха…"></textarea></div><div class="calendar-note-actions"><button type="button" class="calendar-note-cancel" onclick="closeCalendarNoteSheet()">Отмена</button><button type="button" class="sheet-add-btn" onclick="saveCalendarNote()">Сохранить</button></div>`;
  document.getElementById('cal-note-text').value=oldText;
  overlay.classList.add('show'); sheet.classList.add('show'); setTimeout(()=>document.getElementById('cal-note-text')?.focus(),100);
}
...
function saveCalendarNote(){ const iso=document.getElementById('cal-note-date')?.value, text=document.getElementById('cal-note-text')?.value.trim(); if(!iso){toast('Выбери дату');return;} const dateStr=isoToNoteDate(iso), notes=getCalendarNotes(); if(text) notes[dateStr]=text; else delete notes[dateStr]; localStorage.setItem('fs-calendar-notes',JSON.stringify(notes)); closeCalendarNoteSheet(); const [y,m]=iso.split('-'); calCurrentYear=Number(y); calCurrentMonth=Number(m)-1; renderCalendar(); showCalDay(dateStr); toast(text?'Заметка сохранена':'Заметка удалена'); }
function deleteCalendarNote(dateStr){ const notes=getCalendarNotes(); delete notes[dateStr]; localStorage.setItem('fs-calendar-notes',JSON.stringify(notes)); renderCalendar(); showCalDay(dateStr); toast('Заметка удалена'); }
