...
function renderCalendar() {
  ...
  // --- НОВОЕ: Список тренировок за месяц под календарём ---
...
function renderNutrEntries() {
  ...
  if (_circ) {
    // circumference = 2*pi*50 ≈ 314; open gap at bottom ~25% => usable arc 235
...
// item100 = {name, kcal100, p100, c100, f100}
// entryId  = null (новая) или id существующей записи
...
function editNutrEntry(entryId) {
  ...
  if (!entry) return;
  // Восстанавливаем item100 — ищем в FOOD_DB, если нет — вычисляем из граммов
  ...
  if (dbItem) {
    ...
    // Если продукт не из базы — вычисляем из записанных данных и граммов
...
function saveNewRecipe() {
  ...
  // Сохраняем пользовательские рецепты в localStorage
...
// Загружаем пользовательские рецепты из localStorage при старте
