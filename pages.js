...
// FITSIM CALENDAR NOTE SHEET — SWIPE DOWN TO CLOSE
(function () {
  'use strict';
  var boundSheet = null;

  function attachCalendarNoteSwipe() {
    var sheet = document.getElementById('calendar-note-sheet');
    if (!sheet || sheet === boundSheet) return;
    boundSheet = sheet;

    var startX = 0, startY = 0, currentY = 0;
    var dragging = false, vertical = false;
    ...
    function finishSwipe() {
      ...
      if (vertical && dy >= 80) {
        ...
        setTimeout(function () {
          if (typeof window.closeCalendarNoteSheet === 'function') window.closeCalendarNoteSheet();
          else {
            sheet.classList.remove('show');
            document.getElementById('calendar-note-overlay')?.classList.remove('show');
          }
  ...
  // The note sheet is created dynamically when a note is opened.
  var originalOpenCalendarNoteSheet = window.openCalendarNoteSheet;
  if (typeof originalOpenCalendarNoteSheet === 'function') {
    window.openCalendarNoteSheet = function () {
      var result = originalOpenCalendarNoteSheet.apply(this, arguments);
      attachCalendarNoteSwipe();
      return result;
    };
  }

  document.addEventListener('DOMContentLoaded', attachCalendarNoteSwipe);
  window.addEventListener('load', attachCalendarNoteSwipe);
}());
