...
listEl.innerHTML = items.map(function(e) {
      return '<div class="meal-item-wrap" data-id="' + e.id + '">' +
        '<div class="meal-item-delete-btn" onclick="removeNutrEntryById(\'' + e.id + '\')">Удалить</div>' +
        '<div class="meal-item" onclick="editNutrEntry(\'' + e.id + '\')">' +
  ...
  // Uses percentage of daily goal since food-db has no fiber/sugar/sodium fields yet
  // When those fields are added to food-db, replace these estimated values
  const _fillExtra = (id, val, maxVal) => {
    const el = document.getElementById(id);
    if (el) el.textContent = Math.round(val);
    // find corresponding bar fill by proximity in DOM (nutr-extra-list rows)
    ...
    if (idx >= 0 && fills[idx]) {
      ...
      // animate only if slide 2 is visible
      if (window.nutrSlide === 1) fills[idx].style.width = pct.toFixed(1) + '%';
    }
  };
  // Estimated values from current macros (rough approximation until db fields added):
  // fiber ~ 15% of carbs, sugar ~ 40% of carbs, sodium ~ 1mg per kcal,
  // cholesterol ~ 1.5mg per g of fat, potassium ~ 0.5mg per kcal
...
function removeNutrEntryById(id) {
  nutrEntries = nutrEntries.filter(e => String(e.id) !== String(id));
  localStorage.setItem('fs-nutrition', JSON.stringify(nutrEntries));
  renderNutrEntries();
  toast('🗑 Запись удалена');
}
...
window.closeMealPickerSheet = closeMealPickerSheet;t;
...
function openGramsSheet(item100, entryId) {
  ...
  document.documentElement.classList.add('grams-open'); document.body.classList.add('grams-open');
...
function closeGramsSheet() {
  ...
  document.documentElement.classList.remove('grams-open'); document.body.classList.remove('grams-open');
}
...
function changeNutrDate(delta) {
  ...
  // Clamp: don't go into future
  if (window._nutrOffset > 0) window._nutrOffset = 0;
  // Reload nutr entries for the selected date
  renderNutrEntries();
}
window.changeNutrDate = changeNutrDate;
// --- Свайп влево для удаления еды (iOS style) ---
function attachMealSwipeGestures() {
  document.querySelectorAll('.meal-item-wrap').forEach(function(wrap) {
    if (wrap._swipeAttached) return;
    wrap._swipeAttached = true;

    var item = wrap.querySelector('.meal-item');
    var delBtn = wrap.querySelector('.meal-item-delete-btn');
    if (!item || !delBtn) return;
    ...
    function closeThis() {
      isOpen = false;
      item.style.transition = 'transform 0.25s cubic-bezier(0.32,0.72,0,1)';
      item.style.transform = 'translateX(0)';
      delBtn.style.transition = 'transform 0.25s cubic-bezier(0.32,0.72,0,1)';
    ...
    function closeOthers() {
      document.querySelectorAll('.meal-item-wrap').forEach(function(w) {
        if (w !== wrap && w._closeSwipe) w._closeSwipe();
      });
    }

    wrap._closeSwipe = closeThis;

    // Нажатие на кнопку удаления — надёжное событие
    delBtn.addEventListener('click', function(e) {
      e.stopPropagation();
      var id = wrap.getAttribute('data-id');
      item.style.transition = 'transform 0.2s ease';
      item.style.transform = 'translateX(-100vw)';
      wrap.style.transition = 'opacity 0.2s ease, max-height 0.3s ease';
      wrap.style.opacity = '0';
      wrap.style.overflow = 'hidden';
      wrap.style.maxHeight = wrap.offsetHeight + 'px';
      setTimeout(function() { wrap.style.maxHeight = '0'; }, 50);
      setTimeout(function() { removeNutrEntryById(id); }, 320);
    });
    ...
    item.addEventListener('touchstart', function(e) {
      ...
      if (isOpen) {
        // уже открыт — закроем при тапе (touchend без движения)
      }
    }, { passive: true });
    ...
    item.addEventListener('touchmove', function(e) {
      ...
      var base = isOpen ? -maxSwipe : 0;
      var translateX = base + dx;
      // Не даём уйти вправо (translateX > 0) и слишком далеко влево
      if (translateX > 0) translateX = dx * 0.15;
      if (translateX < -maxSwipe * 1.4) translateX = -maxSwipe * 1.4;

      item.style.transform = 'translateX(' + translateX + 'px)';

      // Кнопка двигается обратно пропорционально
      var pct = Math.max(0, Math.min(100, 100 + (translateX / maxSwipe) * 100));
    ...
    item.addEventListener('touchend', function() {
      ...
      if (isHorizontal === null || !isHorizontal) {
        // Чистый тап — если открыт, закрыть
        if (isOpen) closeThis();
        return;
      }
      var dx = currentX - startX;
      if (!isOpen) {
        dx < -30 ? openThis() : closeThis();
      } else {
        dx > 20 ? closeThis() : openThis();
      }
    });
  });
}
