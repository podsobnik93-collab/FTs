...
function openRecipeSheet(recipeId) {
  ...
  const existOv = document.getElementById('recipe-overlay');
  const existSh = document.getElementById('recipe-sheet');
  if (existOv) existOv.remove();
  if (existSh) existSh.remove();

  // оверлей
  const ov = document.createElement('div');
  ov.id = 'recipe-overlay';
  ov.onclick = closeRecipeSheet;
  ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:300;opacity:0;transition:opacity .25s;';
  document.body.appendChild(ov);
  ...
  sh.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 16px 12px;flex-shrink:0;border-bottom:1px solid var(--border)">
      <div style="font-size:1rem;font-weight:800;color:var(--text);flex:1;padding-right:8px">${r.emoji} ${r.name}</div>
      <button id="recipe-close-btn" style="width:32px;height:32px;border-radius:50%;border:none;background:var(--card-inner);color:var(--text-light);font-size:1.1rem;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;line-height:1">✕</button>
    </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ${[
            ...
            <button onclick="addRecipeToMeal('${r.id}','${m.id}')"
              style="padding:11px;border-radius:12px;background:var(--card-inner);border:1.5px solid var(--border);color:var(--text);font-size:0.85rem;font-weight:600;cursor:pointer;transition:background .15s;width:100%">
              ${m.label}
            </button>
          `).join('')}
        </div>
  ...
  // кнопка закрытия — вешаем JS, не onclick в HTML
  ...
  let _startY = 0, _curY = 0, _drag = false;
  sh.addEventListener('touchstart', function(e) {
    if (e.target.closest('button')) return;
    _startY = e.touches[0].clientY;
    _curY = _startY; _drag = true;
    sh.style.transition = 'none';
  }, { passive: true });
  sh.addEventListener('touchmove', function(e) {
    if (!_drag) return;
    _curY = e.touches[0].clientY;
    const dy = Math.max(0, _curY - _startY);
    sh.style.transform = `translate(-50%,calc(-50% + ${dy}px)) scale(1)`;
  }, { passive: true });
  sh.addEventListener('touchend', function() {
    if (!_drag) return; _drag = false;
    const dy = _curY - _startY;
    sh.style.transition = 'opacity .25s,transform .25s cubic-bezier(.34,1.56,.64,1)';
...
function closeRecipeSheet() {
  ...
  const sh = document.getElementById('recipe-sheet');
  if (ov) { ov.style.opacity = '0'; setTimeout(() => { if(ov) ov.remove(); }, 260); }
  if (sh) {
    sh.style.opacity = '0';
    sh.style.transform = 'translate(-50%,-40%) scale(0.92)';
    setTimeout(() => { if(sh) sh.remove(); }, 260);
  }
}
