...
// FITSIM SMOOTH MEAL SLIDER OVERRIDE
(function() {
  'use strict';
  function initFitSimMealSlider() {
    const track = document.getElementById('meal-slides');
    const outer = document.querySelector('.meal-slides-outer');
    if (!track || !outer || track.dataset.fitsimSliderReady === '1') return;
    track.dataset.fitsimSliderReady = '1';
    let active = Math.max(0, Math.min(1, Number(window.mealSlide) || 0));
    let startX = 0, startY = 0, currentX = 0, dragging = false, horizontal = false;
    const ease = 'cubic-bezier(0.16, 1, 0.3, 1)';
    const duration = '0.42s';
    function setDots() {
      document.querySelectorAll('#page-nutrition .meal-dot').forEach((dot, i) => {
        const on = i % 2 === active;
        dot.classList.toggle('meal-dot-active', on);
        dot.setAttribute('aria-pressed', String(on));
      });
    }
    function move(percent, animate) {
      track.style.transition = animate ? `transform ${duration} ${ease}` : 'none';
      track.style.transform = `translate3d(${-percent}%, 0, 0)`;
    }
    function go(index, animate = true) {
      active = Math.max(0, Math.min(1, Number(index) || 0));
      window.mealSlide = active;
      move(active * 50, animate);
      setDots();
      if (active === 1 && typeof window.renderMealAnalytics === 'function') {
        requestAnimationFrame(() => window.renderMealAnalytics());
      }
    }
    window.mealGoTo = go;
    document.querySelectorAll('#page-nutrition .meal-dot').forEach((dot, i) => {
      dot.type = 'button';
      dot.onclick = event => { event.preventDefault(); event.stopPropagation(); go(i % 2); };
    });
    outer.addEventListener('touchstart', event => {
      if (event.touches.length !== 1) return;
      const t = event.touches[0];
      startX = currentX = t.clientX; startY = t.clientY;
    ...
    outer.addEventListener('touchmove', event => {
      ...
      const t = event.touches[0];
      currentX = t.clientX;
      const dx = currentX - startX, dy = t.clientY - startY;
      if (!horizontal) { if (Math.abs(dx) < 8) return; horizontal = Math.abs(dx) > Math.abs(dy); }
      if (!horizontal) return;
      if (event.cancelable) event.preventDefault();
      const width = outer.getBoundingClientRect().width || window.innerWidth;
      const edge = (active === 0 && dx > 0) || (active === 1 && dx < 0);
      const resistance = edge ? 0.28 : 1;
      const percent = active * 50 - (dx / width) * 50 * resistance;
      track.classList.add('is-dragging');
      move(Math.max(0, Math.min(50, percent)), false);
    }, {passive:false});
    function finish() {
      if (!dragging) return;
      const dx = currentX - startX; dragging = false;
      track.classList.remove('is-dragging');
      if (horizontal && Math.abs(dx) >= 44) go(dx < 0 ? active + 1 : active - 1);
      else go(active);
    }
    outer.addEventListener('touchend', finish, {passive:true});
    outer.addEventListener('touchcancel', finish, {passive:true});
    go(active, false);
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initFitSimMealSlider, {once:true});
  else initFitSimMealSlider();
}());
