...
(function() {
  function openRecipeEditorPage(sh){
    var nutrition=document.getElementById('page-nutrition');
    var scroll=document.querySelector('.scroll-area');
    if(nutrition) nutrition.classList.add('recipe-editor-active');
    if(scroll){scroll.dataset.recipeScroll=String(scroll.scrollTop);scroll.scrollTop=0;}
  ...
  window.openAddRecipeSheet = function() {
    var ov = document.getElementById('add-recipe-overlay');
    var sh = document.getElementById('add-recipe-sheet');
    if (sh) { openRecipeEditorPage(sh); return; }
    sh = document.createElement('div');
    sh.id = 'add-recipe-sheet'; sh.className = 'recipe-editor-page';
    sh.innerHTML = `
      <div class="recipe-editor-header"><button type="button" class="recipe-editor-back" onclick="exitRecipeEditor()">‹ Назад</button><div class="sheet-title">Добавить рецепт</div></div>
  ...
  window.closeAddRecipeSheet = function(){ var ov=document.getElementById('add-recipe-overlay'),sh=document.getElementById('add-recipe-sheet'); if(ov)ov.remove(); if(sh)sh.remove(); document.getElementById('page-nutrition')?.classList.remove('recipe-editor-active'); document.body.classList.remove('recipe-editor-open'); var scroll=document.querySelector('.scroll-area'); if(scroll){scroll.scrollTop=Number(scroll.dataset.recipeScroll)||0;delete scroll.dataset.recipeScroll;} };
  window.saveNewRecipe = function(){
