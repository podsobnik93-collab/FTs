...
// FITSIM MEAL PERIOD CONTROLS — RELIABLE FIX
(function () {
  'use strict';

  function normalizePeriod(value) {
    value = String(value || '').trim().toLowerCase();
    if (value === 'day' || value === 'день') return 'day';
    if (value === 'week' || value === 'неделя') return 'week';
    return 'month';
  }

  function changeMealPeriod(period, button) {
    window.mealAnalyticsPeriod = normalizePeriod(period);
    document.querySelectorAll('#page-nutrition .meal-period-btn').forEach(function (item) {
  ...
  function bindMealPeriodControls() {
    // This runs after index.html's legacy inline scripts, so it safely replaces
    // their handler without affecting the meal slider.
    window.setMealAnalyticsPeriod = changeMealPeriod;

    document.querySelectorAll('#page-nutrition .meal-period-btn').forEach(function (button) {
      var period = normalizePeriod(button.getAttribute('data-period') || button.textContent);
      button.setAttribute('data-period', period);
      button.onclick = function (event) {
        event.preventDefault();
        event.stopPropagation();
        changeMealPeriod(period, button);
      };
    });
  }

  // The chart is display-only. It must not take touches away from period buttons.
  function protectMealControls() {
    var styleId = 'fitsim-meal-chart-touch-fix';
    if (document.getElementById(styleId)) return;
    var style = document.createElement('style');
    style.id = styleId;
    style.textContent = '#page-nutrition .meal-macro-chart, #page-nutrition .meal-macro-chart svg { pointer-events: none !important; }';
    document.head.appendChild(style);
  }

  function ready() {
    protectMealControls();
    bindMealPeriodControls();
  }
