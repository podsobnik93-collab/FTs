...
function goTo(id, title) {
  ...
  // Гарантированный сброс прокрутки в начало (с задержкой, чтобы перекрыть возможные изменения высоты)
  setTimeout(() => {
...
function renderHome() {
  ...
  // Сброс прокрутки (на случай, если страница открыта напрямую)
  const scrollArea = document.querySelector('.scroll-area');
  if (scrollArea) scrollArea.scrollTop = 0;
}
...
function renderCalendar() {
  ...
  // Сброс прокрутки в начало страницы
  const scrollArea = document.querySelector('.scroll-area');
  if (scrollArea) scrollArea.scrollTop = 0;
}
