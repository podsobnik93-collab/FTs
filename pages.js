...
function renderDiary() {
  ...
  sorted.forEach(e => {
    ...
    const key = p.length === 3 ? `${p[1]}.${p[2]}` : 'other';
    if (!groups.has(key)) groups.set(key, { label: p.length === 3 ? `${months[Number(p[1])-1]} ${p[2]}` : 'История', entries:[], kcal:0 });
    const g = groups.get(key); g.entries.push(e); g.kcal += Number(e.kcal) || 0;
  });
  let html = '';
  [...groups.values()].forEach((g, gi) => {
    const visible = gi === 0 ? 3 : 0;
    const extra = Math.max(0, g.entries.length - visible);
    html += `<section class="history-month"><div class="history-month-head"><div><div class="history-month-title">${g.label}</div><div class="history-month-meta">${g.entries.length} ${g.entries.length === 1 ? 'тренировка' : g.entries.length < 5 ? 'тренировки' : 'тренировок'}${g.kcal ? ` · ${g.kcal} ккал` : ''}</div></div></div><div class="history-rows" id="history-month-${gi}">`;
    g.entries.forEach((e, i) => {
      const p = (e.date || '').split(',')[0].trim().split('.');
      const day = p[0] || '—';
      const weekday = p.length === 3 ? new Date(`${p[2]}-${p[1]}-${p[0]}T00:00:00`).toLocaleDateString('ru-RU', {weekday:'short'}).replace('.', '') : '';
      const hidden = i >= visible ? ' history-row-hidden' : '';
      const exerciseCount = (e.exercises || '').split('\n').filter(Boolean).length;
      html += `<button class="history-row${hidden}" type="button" onclick="openDiaryDetail('${e.id}')"><span class="history-date"><b>${day}</b><small>${weekday}</small></span><span class="history-main"><b>${e.type || 'Тренировка'}</b><small>${exerciseCount ? `${exerciseCount} упр.` : 'Без упражнений'}${e.kcal ? ` · ${e.kcal} ккал` : ''}</small></span><span class="history-arrow">›</span></button>`;
    });
    html += `</div>${extra ? `<button class="history-more" id="history-more-${gi}" onclick="showMoreHistory(${gi})">Показать ещё ${extra}</button>` : ''}</section>`;
  });
  dl.innerHTML = html;
  renderBodyLog();
}
