...
function renderCalendar() {
  ...
  for(let d=1;d<=daysInMonth;d++){
    ...
    html+=`<button type="button" onclick="showCalDay('${dateStr}')" class="cal-cell${today?' cal-cell-today':''}${wt||isPlanned?' has-workout':''}${food?' has-food':''}${water?' has-water':''}${note?' has-note':''}" aria-label="${dateStr}"><span>${d}</span><b class="cal-marks">${marks}</b></button>`;
  }
  grid.innerHTML=html;
