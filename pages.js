...
function goTo(id, title) {
  ...
  if (id === 'workout') {
    ...
    if (restored && currentWtScreen === 'execute' && customWt.length) {
      ...
      else if (typeof renderWtHome === 'function') renderWtHome();
    }
  }
...
function renderDiary() {
  ...
  if (!diary.length) {
    dl.innerHTML = '<div class="empty-state"></div>';
    renderWlog();
    renderBodyLog();
    return;
  }
  ...
  sorted.forEach(e => {
    ...
    if (!groups[key]) {
      groups[key] = { entries: [], label: '', totalKcal: 0 };
      groupOrder.push(key);
      if (parts.length === 3) {
        const m = parseInt(parts[1], 10) - 1;
        const y = parts[2];
        groups[key].label = `${monthNames[m] || parts[1]} ${y}`;
      } else {
        groups[key].label = key;
      }
    }
    groups[key].entries.push(e);
    groups[key].totalKcal += Number(e.kcal) || 0;
  });

  const openKey = groupOrder[0];
  let html = '';

  groupOrder.forEach((key, gi) => {
    const g = groups[key];
    const isOpen = key === openKey;
    const contentId = `dm-content-${gi}`;
    const arrowId   = `dm-arrow-${gi}`;
    const cnt = g.entries.length;
    const cntLabel = cnt === 1 ? '\u0442\u0440\u0435\u043d\u0438\u0440\u043e\u0432\u043a\u0430' : cnt < 5 ? '\u0442\u0440\u0435\u043d\u0438\u0440\u043e\u0432\u043a\u0438' : '\u0442\u0440\u0435\u043d\u0438\u0440\u043e\u0432\u043e\u043a';
    const totalKcalStr = g.totalKcal > 0
      ? `<span style="font-size:11px;color:var(--accent,#a78bfa);font-weight:700;margin-left:8px">${g.totalKcal} \u043a\u043a\u0430\u043b</span>`
      : '';
    const countStr = `<span style="font-size:11px;color:var(--text-muted,#888);margin-left:6px">${cnt} ${cntLabel}</span>`;

    html += `<div class="diary-month-group" style="margin-bottom:4px;border-radius:14px;overflow:hidden;background:var(--card-inner,rgba(255,255,255,0.04))">
      <div class="diary-month-header"
           ...
           onclick="(function(btn,cnt){const open=cnt.style.display!=='none';cnt.style.display=open?'none':'block';btn.style.transform=open?'rotate(0deg)':'rotate(180deg)'})(document.getElementById('${arrowId}'),document.getElementById('${contentId}'))">
        <div style="display:flex;align-items:center;gap:4px">
          <span style="font-size:14px;font-weight:700;color:var(--text,#e0e0e0)">${g.label}</span>
          ${countStr}${totalKcalStr}
        </div>
        <span id="${arrowId}" style="font-size:10px;color:var(--text-muted,#888);transition:transform 0.2s;display:inline-block;transform:${isOpen?'rotate(180deg)':'rotate(0deg)'}">&#9660;</span>
      </div>
      <div id="${contentId}" style="display:${isOpen?'block':'none'}">`;

    g.entries.forEach((e, i) => {
      const detailId  = `dd-${e.id}-${gi}-${i}`;
      const arrowEntId = `di-${e.id}-${gi}-${i}`;
      const dayPart = (e.date || '').split(',')[0].trim();

      html += `<div class="diary-entry" style="cursor:pointer;border-top:1px solid var(--border,rgba(255,255,255,0.07));padding:10px 14px"
             onclick="(function(){const d=document.getElementById('${detailId}');const a=document.getElementById('${arrowEntId}');if(d.style.display==='none'){d.style.display='block';if(a)a.style.transform='rotate(180deg)'}else{d.style.display='none';if(a)a.style.transform='rotate(0deg)'}})()">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <div style="display:flex;align-items:center;gap:12px">
              <div class="diary-title" style="margin:0;font-size:15px">${e.type}</div>
              <div class="diary-date" style="margin:0;font-size:12px;margin-top:2px;color:var(--text-muted,#888)">${dayPart}</div>
            </div>
            <div style="display:flex;align-items:center;gap:16px">
              <span id="${arrowEntId}" style="font-size:10px;color:var(--text-muted,#888);transition:transform 0.2s;display:inline-block">&#9660;</span>
              <button class="diary-del" onclick="event.stopPropagation();delDiary('${e.id}')" style="margin:0"></button>
            </div>
          </div>
          <div id="${detailId}" style="display:none;margin-top:12px;padding-top:12px;border-top:1px dashed var(--border,rgba(255,255,255,0.1))">
            <div class="diary-date" style="margin-bottom:8px">${e.date}</div>
            <div class="diary-exs" style="margin-bottom:8px;line-height:1.4">${(e.exercises||'').split(', ').join('<br>')}</div>
            ${e.kcal?`<span class="badge green">${e.kcal} \u043a\u043a\u0430\u043b</span>`:''}
          </div>
        </div>`;
    });
...
function renderWtHome() {
  ...
  const diaryDates = new Set(
    (Array.isArray(diary) ? diary : []).map(e => e.date ? e.date.split(',')[0].trim() : '')
  );
  ...
  // --- Последние 7 дней (Пн..Вс текущей недели) ---
  // Строим массив: индекс 0 = 6 дней назад ... индекс 6 = сегодня
  const weekDates = [];
  for (let i = 6; i >= 0; i--) {
  ...
  const weekWorkouts = weekDates.map(ds => diaryDates.has(ds));
  const weekCount = weekWorkouts.filter(Boolean).length;
  const totalCount = Array.isArray(diary) ? diary.length : 0;
  const weekKcal = (Array.isArray(diary) ? diary : [])
    .filter(e => weekDates.includes(e.date ? e.date.split(',')[0].trim() : ''))
    .reduce((sum, e) => sum + (parseInt(e.kcal) || 0), 0);

  // --- DOM updates ---
  const sv = document.getElementById('wth-streak-val');
  if (sv) sv.textContent = streak;
  const wc = document.getElementById('wth-week-count');
  if (wc) wc.textContent = weekCount;
  const tc = document.getElementById('wth-total-count');
  if (tc) tc.textContent = totalCount;
  const wk = document.getElementById('wth-week-kcal');
  if (wk) wk.textContent = weekKcal > 0 ? (weekKcal >= 1000 ? Math.round(weekKcal/100)/10+'k' : weekKcal) : '—';

  const sub = document.getElementById('wth-subtitle');
  if (sub) {
    if (streak >= 7) sub.textContent = '🔥 Неделя без пропусков!';
    else if (streak >= 3) sub.textContent = `💪 ${streak} дня подряд — так держать!`;
    else if (weekCount === 0) sub.textContent = 'Самое время начать 💪';
    else sub.textContent = `${weekCount} из 7 дней на этой неделе`;
  }

  // --- Точки недели ---
  const dotsEl = document.getElementById('wth-week-dots');
  if (dotsEl) {
    const dayLabels = ['Пн','Вт','Ср','Чт','Пт','Сб','Вс'];
    dotsEl.innerHTML = weekWorkouts.map((done, i) => {
      const isToday = i === 6;
      const bg = done ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.2)';
      const border = isToday ? '2px solid rgba(255,255,255,0.9)' : '2px solid transparent';
      return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:3px">
        <div style="width:26px;height:26px;border-radius:50%;background:${bg};border:${border};display:flex;align-items:center;justify-content:center;font-size:11px">${done ? '✓' : ''}</div>
        <div style="font-size:9px;color:rgba(255,255,255,${isToday?'.95':'.55'})">${dayLabels[i]}</div>
      </div>`;
    }).join('');
  }

  // --- Сохранённые тренировки ---
  const savedCard = document.getElementById('saved-wt-card');
  const savedList = document.getElementById('saved-wt-list');
  const savedCount = document.getElementById('saved-wt-count');
  if (savedList && Array.isArray(savedWts) && savedWts.length > 0) {
    if (savedCard) savedCard.style.display = '';
    if (savedCount) savedCount.textContent = savedWts.length + ' шт.';
    savedList.innerHTML = savedWts.map((wt, i) => {
      const exCount = wt.exercises ? wt.exercises.length : 0;
      const muscles = [...new Set((wt.exercises||[]).map(e => e.cat||'').filter(Boolean))].slice(0,3).join(', ');
      return `<div style="flex-shrink:0;width:140px;background:var(--card-inner,rgba(255,255,255,0.05));border:1px solid var(--border);border-radius:14px;padding:12px;display:flex;flex-direction:column;gap:6px;cursor:pointer" onclick="startSavedWorkout(${i})">
        <div style="font-size:13px;font-weight:700;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${wt.name || 'Тренировка'}</div>
        <div style="font-size:11px;color:var(--text-muted);line-height:1.3">${exCount} упр.${muscles ? '<br>'+muscles : ''}</div>
        <div style="margin-top:auto;display:flex;gap:6px">
          <button onclick="event.stopPropagation();startSavedWorkout(${i})" style="flex:1;padding:6px 0;background:var(--accent);color:#fff;border:none;border-radius:8px;font-size:11px;font-weight:700;cursor:pointer">▶ Начать</button>
          <button onclick="event.stopPropagation();deleteSavedWt(${i})" style="width:28px;padding:6px 0;background:var(--input-bg);color:var(--text-muted);border:1px solid var(--border);border-radius:8px;font-size:11px;cursor:pointer">✕</button>
        </div>
      </div>`;
    }).join('');
  } else {
...
window.renderWtHome = renderWtHome;

// ==================== SAVED WORKOUT HELPERS ====================
window.deleteSavedWt = function(idx) {
  if (!confirm('Удалить эту тренировку?')) return;
  if (!Array.isArray(savedWts)) return;
...
if (!window.startSavedWorkout) {
  window.startSavedWorkout = function(idx) {
    if (!Array.isArray(savedWts) || !savedWts[idx]) return;
    ...
    if (typeof renderExecute === 'function') {
      showWtScreen('execute');
      renderExecute();
