...
function openRecipeSheet(recipeId) {
  ...
  if (!r) return;
  let ov = document.getElementById('recipe-overlay');
  let sh = document.getElementById('recipe-sheet');
  if (!ov) {
    ov = document.createElement('div');
  ...
  sh.innerHTML = `
    <div class="sheet-handle"></div>
    <div style="padding:0 20px 20px;overflow-y:auto;max-height:70vh">
      <div style="font-size:2.8rem;text-align:center;margin-bottom:6px">${r.emoji}</div>
      <div style="font-size:1.1rem;font-weight:800;color:var(--text);text-align:center;margin-bottom:4px">${r.name}</div>
      ...
      <div style="font-size:0.75rem;font-weight:700;color:var(--text-light);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">Ингредиенты</div>
      ...
      <div style="margin-top:16px;display:flex;flex-direction:column;gap:8px">
        <div style="font-size:0.75rem;font-weight:700;color:var(--text-light);text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px">Добавить в приём пищи</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ${[
            ...
            <button onclick="addRecipeToMeal('${r.id}','${m.id}')"
              style="padding:11px;border-radius:12px;background:var(--card-inner);border:1.5px solid var(--border);color:var(--text);font-size:0.85rem;font-weight:600;cursor:pointer;transition:background .15s">
              ${m.label}
            </button>
          `).join('')}
        </div>
