...
function showCalDay(dateStr) {
  ...
  if(dayDiary.length){
    html+=`<section class="cal-detail-block workout"><div class="cal-detail-head"><span>Тренировка</span><small>${dayDiary.length} ${dayDiary.length===1?'запись':'записи'}</small></div>`;
    dayDiary.forEach(d=>{const exCount=(d.exercises||'').split('\n').filter(Boolean).length;html+=`<article class="cal-detail-row"><div class="cal-detail-copy"><b>${d.type||'Тренировка'}</b><small>${exCount?`${exCount} упр.`:'Без упражнений'}${d.kcal?` · ${d.kcal} ккал`:''}</small></div><div class="cal-detail-actions"><button type="button" onclick="editDiaryFromCal('${d.id}','${dateStr}')" aria-label="Изменить">Изм.</button><button type="button" class="danger" onclick="delDiaryFromCal('${d.id}','${dateStr}')" aria-label="Удалить">Удалить</button></div></article>`});html+='</section>';
  }
  if(dayNutr.length){
    const foodId=`cal-food-${dateStr.replaceAll('.','-')}`;
    html+=`<section class="cal-detail-block food"><button class="cal-card-toggle" type="button" aria-expanded="false" onclick="toggleCalCard('${foodId}',this)"><span class="cal-card-title">Питание</span><span class="cal-card-chevron">⌄</span></button><div class="cal-card-body" id="${foodId}" hidden><div class="cal-food-summary">${Math.round(totalKcal)} ккал · Б ${Math.round(totalProt)} · Ж ${Math.round(totalFat)} · У ${Math.round(totalCarbs)}</div>`;
  ...
  if(waterAmount>0){const waterId=`cal-water-${dateStr.replaceAll('.','-')}`;html+=`<section class="cal-detail-block water"><button class="cal-card-toggle" type="button" aria-expanded="false" onclick="toggleCalCard('${waterId}',this)"><span class="cal-card-title">Вода</span><span class="cal-card-chevron">⌄</span></button><div class="cal-card-body" id="${waterId}" hidden><div class="cal-water"><b>${waterAmount}</b><span>из ${waterGoal} мл</span><em>${waterPct}%</em></div><div class="cal-water-bar"><i style="width:${waterPct}%"></i></div></div></section>`;}
  entries.innerHTML=html;
  if(detail)detail.style.display=html?'block':'none';
}
window.showCalDay=showCalDay;

function toggleCalCard(id, button){
  const body=document.getElementById(id); if(!body)return;
  const open=body.hasAttribute('hidden');
  if(open) body.removeAttribute('hidden'); else body.setAttribute('hidden','');
  const card=button.closest('.cal-detail-block');
  button.setAttribute('aria-expanded',String(open));
  if(card) card.classList.toggle('is-open',open);
}
window.toggleCalCard=toggleCalCard;
