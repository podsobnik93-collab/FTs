...
function attachMealSwipeGestures() {
  document.querySelectorAll('.meal-item-wrap').forEach(function(wrap) {
    ...
    var isOpen = false;
    // Кнопка удаления обычно шириной около 80-90px
    var maxSwipe = 85; 

    item.addEventListener('touchstart', function(e) {
      // Закрываем остальные открытые
      document.querySelectorAll('.meal-item-wrap').forEach(w => {
        if (w !== wrap) {
          var otherBtn = w.querySelector('.meal-item-delete-btn');
          var otherItem = w.querySelector('.meal-item');
          if (otherBtn && otherItem && otherBtn.style.transform === 'translateX(0%)') {
            otherItem.style.transform = 'translateX(0)';
            otherBtn.style.transform = 'translateX(100%)';
            // We can't directly change isOpen of other closures, but it will reset on next touch anyway
          }
        }
      });
      ...
      // Если кликнули, а свайп открыт - закроем
      if (isOpen) {
        isOpen = false;
        item.style.transition = 'transform 0.25s ease';
        item.style.transform = 'translateX(0)';
        delBtn.style.transition = 'transform 0.25s ease';
    ...
    item.addEventListener('touchmove', function(e) {
      ...
      if (isHorizontal) {
        var translateX = dx;
        if (isOpen) {
          translateX = -maxSwipe + dx;
        }
        if (translateX > 0) translateX = Math.max(translateX, 0) * 0.2; 
        if (translateX < -maxSwipe * 1.5) translateX = -maxSwipe * 1.5;

        item.style.transform = 'translateX(' + translateX + 'px)';

        // Двигаем кнопку удаления
        var btnTranslate = 100 - (Math.abs(Math.min(0, translateX)) / maxSwipe * 100);
        btnTranslate = Math.max(0, btnTranslate);
        delBtn.style.transition = 'none';
        delBtn.style.transform = 'translateX(' + btnTranslate + '%)';
