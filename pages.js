...
/* FITSIM — редактирование и удаление рецептов */
(function () {
  var deletedKey = 'fs-deleted-recipe-ids';
  var overridesKey = 'fs-recipe-overrides';
  ...
  function applyRecipeStorage() {
    var deleted = readJSON(deletedKey, []), overrides = readJSON(overridesKey, {});
    for (var i = RECIPES.length - 1; i >= 0; i--) {
      if (deleted.indexOf(RECIPES[i].id) !== -1) RECIPES.splice(i, 1);
      else if (overrides[RECIPES[i].id]) Object.assign(RECIPES[i], overrides[RECIPES[i].id]);
    }
  }
  applyRecipeStorage();

  var pencilSvg = '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>';
  var trashSvg = '<svg viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v5M14 11v5"/></svg>';
  var isRecipesEditing = false;

  function recipeEditButton(action, id, icon, color, label) {
    return '<button type="button" aria-label="' + label + '" title="' + label + '" onclick="event.stopPropagation();' + action + '(\'' + id + '\')" style="width:36px;height:36px;display:inline-flex;align-items:center;justify-content:center;padding:0;border:1px solid var(--border);border-radius:10px;background:var(--input-bg);color:' + color + ';cursor:pointer;">' + icon + '</button>';
  }

  window.openRecipesPanel = openRecipesPanel = function () {
    var ov = document.getElementById('recipes-panel-overlay');
    var sh = document.getElementById('recipes-panel-sheet');
    if (ov || sh) { if (typeof closeRecipesPanel === 'function') closeRecipesPanel(); }
    ov = document.createElement('div'); ov.id = 'recipes-panel-overlay'; ov.className = 'sheet-overlay'; ov.onclick = function () { closeRecipesPanel(); }; document.body.appendChild(ov);
    sh = document.createElement('div'); sh.id = 'recipes-panel-sheet'; sh.className = 'bottom-sheet'; sh.style.maxHeight = '85vh';
    var list = RECIPES.map(function (r) {
      var actions = isRecipesEditing ? '<span style="display:flex;gap:6px;flex-shrink:0;">' + recipeEditButton('openRecipeEditor', r.id, pencilSvg, 'var(--accent)', 'Изменить рецепт') + recipeEditButton('deleteRecipe', r.id, trashSvg, '#ef4444', 'Удалить рецепт') + '</span>' : '';
      return '<div class="recipe-chip" role="button" tabindex="0" onclick="event.stopPropagation();' + (isRecipesEditing ? 'openRecipeEditor(\'' + r.id + '\')' : 'closeRecipesPanel();setTimeout(function(){openRecipeSheet(\'' + r.id + '\')},180)') + '"><span class="recipe-chip-emoji">' + r.emoji + '</span><span class="recipe-chip-name">' + r.name + '</span><span class="recipe-chip-kcal">' + r.kcal + ' ккал</span>' + actions + '</div>';
    }).join('');
    sh.innerHTML = '<div class="sheet-handle"></div><div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:0 16px;margin:0 0 14px;"><div class="sheet-title" style="margin:0;">🔍 Рецепты</div><button type="button" aria-label="Редактировать рецепты" title="Редактировать рецепты" onclick="toggleRecipesEditing()" style="width:42px;height:42px;display:inline-flex;align-items:center;justify-content:center;padding:0;border:1px solid ' + (isRecipesEditing ? 'var(--accent)' : 'var(--border)') + ';border-radius:10px;background:' + (isRecipesEditing ? 'rgba(34,197,94,.14)' : 'var(--input-bg)') + ';color:' + (isRecipesEditing ? 'var(--accent)' : 'var(--text-light)') + ';cursor:pointer;">' + pencilSvg + '</button></div><div style="padding:0 16px 20px;overflow-y:auto;max-height:65vh;"><div class="recipe-chips" style="margin-bottom:12px;">' + list + '</div>' + (isRecipesEditing ? '<div style="font-size:12px;color:var(--text-light);text-align:center;margin:4px 0 12px;">Нажмите на рецепт или на карандаш, чтобы изменить его.</div>' : '') + '<button onclick="closeRecipesPanel();setTimeout(openAddRecipeSheet,180)" style="margin-top:4px;width:100%;padding:11px;background:var(--accent);color:#fff;border:none;border-radius:12px;font-size:.85rem;font-weight:700;cursor:pointer;">+ Добавить рецепт</button></div>';
    document.body.appendChild(sh);
    setTimeout(function () { ov.classList.add('show'); sh.classList.add('show'); }, 10);
  };

  window.toggleRecipesEditing = function () { isRecipesEditing = !isRecipesEditing; closeRecipesPanel(); setTimeout(openRecipesPanel, 180); };
  window.openRecipeEditor = function (id) {
    var recipe = RECIPES.find(function (r) { return r.id === id; }); if (!recipe) return;
    closeRecipesPanel();
    setTimeout(function () {
      openAddRecipeSheet();
      setTimeout(function () {
        var title = document.querySelector('#add-recipe-sheet .sheet-title'); if (title) title.textContent = '✏️ Изменить рецепт';
        var set = function (selector, value) { var el = document.querySelector(selector); if (el) el.value = value == null ? '' : value; };
        set('#new-recipe-name', recipe.name); set('#new-recipe-emoji', recipe.emoji || '🍽️'); set('#new-recipe-kcal', recipe.kcal); set('#new-recipe-time', String(recipe.time || '').replace(/\D/g, '')); set('#new-recipe-protein', recipe.protein); set('#new-recipe-fat', recipe.fat); set('#new-recipe-carbs', recipe.carbs);
        var container = document.getElementById('new-recipe-ingredients'); if (container) { container.innerHTML = ''; (recipe.ingredients || []).forEach(function (ing) { addRecipeIngredientRow(); var row = container.lastElementChild; row.querySelector('.new-recipe-ingredient-name').value = ing.name || ''; row.querySelector('.new-recipe-ingredient-grams').value = ing.grams || ''; }); if (!container.children.length) addRecipeIngredientRow(); }
        var save = document.querySelector('#add-recipe-sheet button[onclick*="saveNewRecipe"]'); if (save) { save.textContent = 'Сохранить изменения'; save.onclick = function () { window.saveRecipeChanges(id); }; }
      }, 30);
    }, 220);
  };
  window.saveRecipeChanges = function (id) {
    var recipe = RECIPES.find(function (r) { return r.id === id; }); if (!recipe) return;
    var name = document.getElementById('new-recipe-name')?.value.trim(); if (!name) { toast('Введите название рецепта'); return; }
    var ingredients = [], rows = document.querySelectorAll('#new-recipe-ingredients > div');
    for (var i = 0; i < rows.length; i++) { var n = rows[i].querySelector('.new-recipe-ingredient-name').value.trim(), raw = rows[i].querySelector('.new-recipe-ingredient-grams').value, g = parseFloat(raw); if (!n && !raw) continue; if (!n) { toast('Введите название ингредиента'); return; } if (!Number.isFinite(g) || g <= 0) { toast('Укажите граммовку для: ' + n); return; } ingredients.push({ name: n, grams: Math.round(g * 10) / 10 }); }
    var mins = parseFloat(document.getElementById('new-recipe-time')?.value) || 0;
    Object.assign(recipe, { name: name, emoji: document.getElementById('new-recipe-emoji')?.value.trim() || '🍽️', time: mins > 0 ? Math.round(mins) + ' мин' : '0 мин', kcal: Math.round(parseFloat(document.getElementById('new-recipe-kcal')?.value) || 0), protein: Math.round(parseFloat(document.getElementById('new-recipe-protein')?.value) || 0), fat: Math.round(parseFloat(document.getElementById('new-recipe-fat')?.value) || 0), carbs: Math.round(parseFloat(document.getElementById('new-recipe-carbs')?.value) || 0), ingredients: ingredients });
    var overrides = readJSON(overridesKey, {}); overrides[id] = recipe; saveJSON(overridesKey, overrides); localStorage.setItem('fs-recipes', JSON.stringify(RECIPES)); closeAddRecipeSheet(); toast('Рецепт обновлён'); setTimeout(openRecipesPanel, 180);
  };
  window.deleteRecipe = function (id) {
    var recipe = RECIPES.find(function (r) { return r.id === id; }); if (!recipe || !confirm('Удалить рецепт «' + recipe.name + '»?')) return;
    var deleted = readJSON(deletedKey, []); if (deleted.indexOf(id) === -1) deleted.push(id); saveJSON(deletedKey, deleted);
    var overrides = readJSON(overridesKey, {}); delete overrides[id]; saveJSON(overridesKey, overrides);
    var index = RECIPES.findIndex(function (r) { return r.id === id; }); if (index !== -1) RECIPES.splice(index, 1); localStorage.setItem('fs-recipes', JSON.stringify(RECIPES)); closeRecipesPanel(); toast('Рецепт удалён'); setTimeout(openRecipesPanel, 180);
  };
})();
