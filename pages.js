...
function updateCtaLabel(){
  ...
  if(currentPage === 'nutrition' || currentPage === 'home' || currentPage === 'calendar'){
...
function buildGramsSheetHTML(item, grams) {
  return '<div class="sheet-handle"></div>' +
    ...
    '<div class="grams-ctrl" style="justify-content:center;gap:10px;">' +
      ...
      '</div>' +
      '<button onclick="toggleVoiceInSheet()" id="sheet-mic-btn" aria-label="Голосовой ввод" style="width:46px;height:46px;border-radius:50%;background:var(--accent);border:none;color:#fff;flex-shrink:0;cursor:pointer;display:flex;align-items:center;justify-content:center;">' +
        '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="2" width="6" height="12" rx="3"/><path d="M5 10a7 7 0 0 0 14 0"/><line x1="12" y1="17" x2="12" y2="21"/><line x1="8" y1="21" x2="16" y2="21"/></svg>' +
      '</button>' +
    '</div>' +
...
// ── Голосовой ввод внутри шторок ──────────────────────────────
function toggleVoiceInSheet() {
  const btn = document.getElementById('sheet-mic-btn');
  if (isListening) {
    stopVoice();
    if (btn) { btn.innerHTML = '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="2" width="6" height="12" rx="3"/><path d="M5 10a7 7 0 0 0 14 0"/><line x1="12" y1="17" x2="12" y2="21"/><line x1="8" y1="21" x2="16" y2="21"/></svg>'; btn.style.background = 'var(--accent)'; }
    return;
  }
  const SpeechRec = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!SpeechRec) { toast('🎤 Не поддерживается браузером'); return; }
  ...
  rec.onstart = () => {
    isListening = true;
    if (btn) { btn.innerHTML = '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="#fff" stroke-width="2"><rect x="5" y="5" width="14" height="14" rx="2" fill="#fff"/></svg>'; btn.style.background = '#ef4444'; }
    showVoiceResult('🎤 Слушаю...');
  };

  rec.onresult = (e) => {
    const text = e.results[0][0].transcript;
    // Если открыта grams-sheet — вставить число в поле граммов
    const gramsInput = document.getElementById('grams-input');
    const foodSearch = document.getElementById('food-search');
    if (gramsInput && document.getElementById('grams-sheet')) {
      // Извлечь число из фразы (напр. "двести пятьдесят" или "150 грамм")
      const numWords = {
      ...
      let val = null;
      if (digitMatch) {
        val = parseFloat(digitMatch[0].replace(',', '.'));
      } else {
        // Складываем числительные
        let sum = 0;
        lower.split(/\s+/).forEach(w => { if (numWords[w] !== undefined) sum += numWords[w]; });
        if (sum > 0) val = sum;
      }
      if (val && val > 0) {
        gramsInput.value = val;
        recalcGrams(val);
        showVoiceResult('⚖️ ' + val + ' г');
      } else {
        showVoiceResult('❓ ' + text);
      }
    } else if (foodSearch) {
      // food-picker-sheet — искать продукт
...
window.toggleVoiceInSheet = toggleVoiceInSheet;
...
function runSimulation() {
  ...
  if (profile?.goal === 'loss') rec -= 400;
  if (profile?.goal === 'gain') rec += 300;
  const eatPerDay = Math.max(1000, rec + deficit);
  ...
  if (warnEl) {
    const warnings = [];
    if (deficit < -700) warnings.push('⚠️ Дефицит больше 700 ккал/день — риск потери мышц и слабости');
    if (deficit > 700)  warnings.push('⚠️ Профицит больше 700 ккал/день — высокий набор жира');
