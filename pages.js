...
/* FITSIM — плавный режим редактирования рецептов без закрытия списка */
(function () {
  var deletedKey='fs-deleted-recipe-ids', overridesKey='fs-recipe-overrides', recipesEditing=false;
  function getStore(key, fallback){try{return JSON.parse(localStorage.getItem(key)||'')||fallback;}catch(e){return fallback;}}
  function putStore(key, value){localStorage.setItem(key,JSON.stringify(value));}
  var deleted=getStore(deletedKey,[]), overrides=getStore(overridesKey,{});
  for(var i=RECIPES.length-1;i>=0;i--){if(deleted.indexOf(RECIPES[i].id)!==-1)RECIPES.splice(i,1);else if(overrides[RECIPES[i].id])Object.assign(RECIPES[i],overrides[RECIPES[i].id]);}
  var pencil='<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>';
  var trash='<svg viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v5M14 11v5"/></svg>';
  function iconButton(handler, id, icon, color, label){return '<button type="button" aria-label="'+label+'" title="'+label+'" onclick="event.stopPropagation();'+handler+'(\''+id+'\')" style="width:36px;height:36px;display:inline-flex;align-items:center;justify-content:center;padding:0;border:1px solid var(--border);border-radius:10px;background:var(--input-bg);color:'+color+';cursor:pointer;">'+icon+'</button>';}
  function renderRecipesPanel(sh){
    var rows=RECIPES.map(function(r){var controls=recipesEditing?'<span style="display:flex;gap:6px;flex-shrink:0;">'+iconButton('openRecipeEditor',r.id,pencil,'var(--accent)','Изменить рецепт')+iconButton('deleteRecipe',r.id,trash,'#ef4444','Удалить рецепт')+'</span>':'';var click=recipesEditing?'openRecipeEditor(\''+r.id+'\')':'closeRecipesPanel();setTimeout(function(){openRecipeSheet(\''+r.id+'\')},180)';return '<div class="recipe-chip" role="button" tabindex="0" onclick="event.stopPropagation();'+click+'"><span class="recipe-chip-emoji">'+r.emoji+'</span><span class="recipe-chip-name">'+r.name+'</span><span class="recipe-chip-kcal">'+r.kcal+' ккал</span>'+controls+'</div>';}).join('');
    sh.innerHTML='<div class="sheet-handle"></div><div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:0 16px;margin:0 0 14px;"><div class="sheet-title" style="margin:0;">🔍 Рецепты</div><button type="button" aria-label="Редактировать рецепты" title="Редактировать рецепты" onclick="toggleRecipesEditing()" style="width:42px;height:42px;display:inline-flex;align-items:center;justify-content:center;padding:0;border:1px solid '+(recipesEditing?'var(--accent)':'var(--border)')+';border-radius:10px;background:'+(recipesEditing?'rgba(34,197,94,.14)':'var(--input-bg)')+';color:'+(recipesEditing?'var(--accent)':'var(--text-light)')+';cursor:pointer;transition:background .18s,border-color .18s,color .18s;">'+pencil+'</button></div><div style="padding:0 16px 20px;overflow-y:auto;max-height:65vh;"><div class="recipe-chips" style="margin-bottom:12px;">'+rows+'</div>' +(recipesEditing?'<div style="font-size:12px;color:var(--text-light);text-align:center;margin:4px 0 12px;">Нажмите на рецепт, чтобы изменить его.</div>':'')+'<button onclick="closeRecipesPanel();setTimeout(openAddRecipeSheet,180)" style="margin-top:4px;width:100%;padding:11px;background:var(--accent);color:#fff;border:0;border-radius:12px;font-size:.85rem;font-weight:700;cursor:pointer;">+ Добавить рецепт</button></div>';
  }
  window.openRecipesPanel=openRecipesPanel=function(){var ov=document.getElementById('recipes-panel-overlay'),sh=document.getElementById('recipes-panel-sheet');if(ov&&sh){renderRecipesPanel(sh);return;}ov=document.createElement('div');ov.id='recipes-panel-overlay';ov.className='sheet-overlay';ov.onclick=function(){closeRecipesPanel();};document.body.appendChild(ov);sh=document.createElement('div');sh.id='recipes-panel-sheet';sh.className='bottom-sheet';sh.style.maxHeight='85vh';renderRecipesPanel(sh);document.body.appendChild(sh);setTimeout(function(){ov.classList.add('show');sh.classList.add('show');},10);};
  window.toggleRecipesEditing=function(){recipesEditing=!recipesEditing;var sh=document.getElementById('recipes-panel-sheet');if(sh)renderRecipesPanel(sh);};
  window.openRecipeEditor=function(id){var r=RECIPES.find(function(x){return x.id===id;});if(!r)return;closeRecipesPanel();setTimeout(function(){openAddRecipeSheet();setTimeout(function(){var title=document.querySelector('#add-recipe-sheet .sheet-title');if(title)title.textContent='✏️ Изменить рецепт';function set(sel,v){var e=document.querySelector(sel);if(e)e.value=v==null?'':v;}set('#new-recipe-name',r.name);set('#new-recipe-emoji',r.emoji||'🍽️');set('#new-recipe-kcal',r.kcal);set('#new-recipe-time',String(r.time||'').replace(/\D/g,''));set('#new-recipe-protein',r.protein);set('#new-recipe-fat',r.fat);set('#new-recipe-carbs',r.carbs);var box=document.getElementById('new-recipe-ingredients');if(box){box.innerHTML='';(r.ingredients||[]).forEach(function(ing){addRecipeIngredientRow();var row=box.lastElementChild;row.querySelector('.new-recipe-ingredient-name').value=ing.name||'';row.querySelector('.new-recipe-ingredient-grams').value=ing.grams||'';});if(!box.children.length)addRecipeIngredientRow();}var save=document.querySelector('#add-recipe-sheet button[onclick*="saveNewRecipe"]');if(save){save.textContent='Сохранить изменения';save.onclick=function(){saveRecipeChanges(id);};}},30);},280);};
  window.saveRecipeChanges=function(id){var r=RECIPES.find(function(x){return x.id===id;});if(!r)return;var name=document.getElementById('new-recipe-name')?.value.trim();if(!name){toast('Введите название рецепта');return;}var ingredients=[],rows=document.querySelectorAll('#new-recipe-ingredients > div');for(var i=0;i<rows.length;i++){var n=rows[i].querySelector('.new-recipe-ingredient-name').value.trim(),raw=rows[i].querySelector('.new-recipe-ingredient-grams').value,g=parseFloat(raw);if(!n&&!raw)continue;if(!n){toast('Введите название ингредиента');return;}if(!Number.isFinite(g)||g<=0){toast('Укажите граммовку для: '+n);return;}ingredients.push({name:n,grams:Math.round(g*10)/10});}var min=parseFloat(document.getElementById('new-recipe-time')?.value)||0;Object.assign(r,{name:name,emoji:document.getElementById('new-recipe-emoji')?.value.trim()||'🍽️',time:min>0?Math.round(min)+' мин':'0 мин',kcal:Math.round(parseFloat(document.getElementById('new-recipe-kcal')?.value)||0),protein:Math.round(parseFloat(document.getElementById('new-recipe-protein')?.value)||0),fat:Math.round(parseFloat(document.getElementById('new-recipe-fat')?.value)||0),carbs:Math.round(parseFloat(document.getElementById('new-recipe-carbs')?.value)||0),ingredients:ingredients});var all=getStore(overridesKey,{});all[id]=r;putStore(overridesKey,all);localStorage.setItem('fs-recipes',JSON.stringify(RECIPES));closeAddRecipeSheet();toast('Рецепт обновлён');setTimeout(openRecipesPanel,180);};
  window.deleteRecipe=function(id){var r=RECIPES.find(function(x){return x.id===id;});if(!r||!confirm('Удалить рецепт «'+r.name+'»?'))return;var d=getStore(deletedKey,[]);if(d.indexOf(id)===-1)d.push(id);putStore(deletedKey,d);var o=getStore(overridesKey,{});delete o[id];putStore(overridesKey,o);var index=RECIPES.findIndex(function(x){return x.id===id;});if(index!==-1)RECIPES.splice(index,1);localStorage.setItem('fs-recipes',JSON.stringify(RECIPES));var sh=document.getElementById('recipes-panel-sheet');if(sh)renderRecipesPanel(sh);toast('Рецепт удалён');};
})();
