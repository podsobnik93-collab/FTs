...
function renderNutrEntries() {
  ...
  list.innerHTML = todayEntries.length
    ? todayEntries.map((e, i) => `
      ...
      `).join('')
    : '<div style="text-align:center;padding:24px 10px;color:var(--text-light);display:flex;flex-direction:column;align-items:center;gap:12px;">' +
      '<div style="width:48px;height:48px;border-radius:14px;background:var(--input-bg);display:flex;align-items:center;justify-content:center;font-size:24px;border:1px dashed var(--border);">🍽️</div>' +
      '<div style="font-size:13px;font-weight:500;">Журнал пуст</div>' +
      '<div style="font-size:11px;opacity:0.7;">Записи появятся после добавления приема пищи</div>' +
      '</div>';
