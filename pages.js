...
// FITSIM REMOVE LIGHT THEME
// The light theme has been retired. Existing saved choices are moved to Arctic.
(function () {
  var legacyTheme = localStorage.getItem('fs-theme');
  if (legacyTheme === 'light') {
    localStorage.setItem('fs-theme', 'arctic');
    document.documentElement.setAttribute('data-theme', 'arctic');
  }

  var previousToggleTheme = window.toggleTheme;
  window.toggleTheme = function () {
    var order = ['dark', 'arctic', 'carbon'];
    var current = localStorage.getItem('fs-theme') || 'dark';
    if (current === 'light') current = 'arctic';
    var next = order[(order.indexOf(current) + 1) % order.length];
    localStorage.setItem('fs-theme', next);
    document.documentElement.setAttribute('data-theme', next);
    var icon = { dark: '🌙', arctic: '❄️', carbon: '◼️' };
    var button = document.getElementById('theme-btn');
    if (button) button.textContent = icon[next];
    if (typeof window.toast === 'function') {
      window.toast(next === 'dark' ? 'Тёмная тема' : (next === 'arctic' ? 'Холод' : 'Carbon'));
    }
  };
}());
