...
let voiceResultHideTimer = null;

function showVoiceResult(text) {
  const el = document.getElementById('voice-result');
  if (!el) return;

  if (voiceResultHideTimer) {
    clearTimeout(voiceResultHideTimer);
    voiceResultHideTimer = null;
  }

  el.textContent = text;
  el.classList.add('show');

  voiceResultHideTimer = setTimeout(() => {
    hideVoiceResult();
  }, 2500);
}
