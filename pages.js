...
function updateCtaLabel(){
  ...
  if(currentPage === 'nutrition' || currentPage === 'home' || currentPage === 'calendar' || currentPage === 'ai' || currentPage === 'diary'){
...
function renderDiary() {
  ...
  if (!diary.length) {
    dl.innerHTML = '<div class="empty-state">У вас пока нет тренировок</div>';
    renderWlog(); renderBodyLog(); return;
  }

  const sorted = [...diary].sort((a, b) => {
    const toISO = s => { const p=(s||'').split(',')[0].trim().split('.'); return p.length===3?`${p[2]}-${p[1]}-${p[0]}`:s; };
  ...
  sorted.forEach(e => {
    ...
    if (!groups[key]) { groups[key]={entries:[],label:'',totalKcal:0}; groupOrder.push(key); }
    if (parts.length===3) groups[key].label=`${mNames[parseInt(parts[1],10)-1]} ${parts[2]}`;
  ...
  groupOrder.forEach((key,gi) => {
    const g=groups[key], isOpen=(gi===0);
    const cId=`dm-c-${gi}`, aId=`dm-a-${gi}`;
    const cnt=g.entries.length;
    const cntLbl=cnt===1?'тренировка':cnt<5?'тренировки':'тренировок';
    const kcalStr=g.totalKcal>0?`<span style="font-size:11px;color:var(--accent,#a78bfa);font-weight:700;margin-left:8px;">~${g.totalKcal} ккал</span>`:'';
    html+=`<div style="margin-bottom:4px;border-radius:14px;overflow:hidden;background:var(--card-inner,rgba(255,255,255,0.04));">
      <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 14px;cursor:pointer;user-select:none;"
        onclick="(function(a,c){const o=c.style.display!=='none';c.style.display=o?'none':'block';a.style.transform=o?'rotate(0deg)':'rotate(180deg)';})(document.getElementById('${aId}'),document.getElementById('${cId}'))">
        <div style="display:flex;align-items:center;gap:4px;">
          <span style="font-size:14px;font-weight:700;color:var(--text,#e0e0e0);">${g.label}</span>
          <span style="font-size:11px;color:var(--text-muted,#888);margin-left:6px;">${cnt} ${cntLbl}</span>${kcalStr}
        </div>
        <span id="${aId}" style="font-size:10px;color:var(--text-muted,#888);transition:transform 0.2s;display:inline-block;transform:${isOpen?'rotate(180deg)':'rotate(0deg)'};">&#9660;</span>
      </div>
      <div id="${cId}" style="display:${isOpen?'block':'none'};">`;

    g.entries.forEach((e,i) => {
      const dId=`dd-${gi}-${i}`, arId=`di-${gi}-${i}`;
      const day=(e.date||'').split(',')[0].trim();
      html+=`<div style="cursor:pointer;border-top:1px solid var(--border,rgba(255,255,255,0.07));padding:10px 14px;"
        onclick="(function(){const d=document.getElementById('${dId}'),a=document.getElementById('${arId}');if(d.style.display==='none'){d.style.display='block';if(a)a.style.transform='rotate(180deg)';}else{d.style.display='none';if(a)a.style.transform='rotate(0deg)';}})()">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <div style="display:flex;align-items:center;gap:12px;">
            <div class="diary-title" style="margin:0;font-size:15px;">${e.type||'Тренировка'}</div>
            <div class="diary-date" style="margin:0;font-size:12px;color:var(--text-muted,#888);">${day}</div>
          </div>
          <div style="display:flex;align-items:center;gap:16px;">
            <span id="${arId}" style="font-size:10px;color:var(--text-muted,#888);transition:transform 0.2s;display:inline-block;">&#9660;</span>
            <button class="diary-del" onclick="event.stopPropagation();delDiary('${e.id}')" style="margin:0;">✕</button>
          </div>
        </div>
        <div id="${dId}" style="display:none;margin-top:12px;padding-top:12px;border-top:1px dashed var(--border,rgba(255,255,255,0.1));">
          <div class="diary-date" style="margin-bottom:8px;">${e.date}</div>
          <div class="diary-exs" style="margin-bottom:8px;line-height:1.4;">${(e.exercises||'').split('\n').join('<br>')}</div>
...
function renderWlog() {
  // Данные веса теперь отображаются вместе с замерами в renderBodyLog.
}
...
function renderBodyLog() {
  ...
  if (!entries.length) {
    c.innerHTML = '<div class="empty-state" style="color:var(--text-light);font-size:0.85rem;padding:4px 0 8px">Нет записей. Нажми «+ Добавить», чтобы внести вес или замеры.</div>';
    renderBodyChart();
    return;
  }
  c.innerHTML = entries.map((e, i) => {
    ...
    if (e.weight !== undefined && e.weight !== null && e.weight !== '') {
      let diffHtml = '';
      if (previous && previous.weight !== undefined) {
        const diff = +(Number(e.weight) - Number(previous.weight)).toFixed(1);
        if (diff !== 0) diffHtml = `<div class="wlog-diff ${diff < 0 ? 'neg' : 'pos'}">${diff > 0 ? '+' : ''}${diff} кг к пред.</div>`;
      }
      weightHtml = `<div class="body-weight"><span class="body-weight-label">Вес</span><strong>${e.weight} кг</strong>${diffHtml}</div>`;
    }
    let gridHtml = '';
    Object.keys(labels).forEach(k => {
      if (e[k] !== undefined && e[k] !== null && e[k] !== '') {
        let diffHtml = '';
        if (previous && previous[k] !== undefined && previous[k] !== null) {
          const diff = +(Number(e[k]) - Number(previous[k])).toFixed(1);
          if (diff !== 0) diffHtml = `<div style="font-size:0.6rem;font-weight:700;color:${diff > 0 ? '#27ae60' : '#e74c3c'}">${diff > 0 ? '+' : ''}${diff}</div>`;
        }
        gridHtml += `<div class="bg-item"><div class="bg-val">${e[k]}</div><div class="bg-lbl">${labels[k]}</div>${diffHtml}</div>`;
      }
    });
    const dateStr = new Date(e.date + 'T00:00:00').toLocaleDateString('ru', { day: 'numeric', month: 'long', year: 'numeric' });
    return `<div class="body-entry"><div style="display:flex;justify-content:space-between;align-items:center;"><div class="wlog-date-text" style="font-weight:700;color:var(--text)">${dateStr}</div><button class="wlog-delete" style="width:26px;height:26px;font-size:0.8rem;" onclick="delBody('${e.date}')">✕</button></div>${weightHtml}${gridHtml ? `<div class="body-grid">${gridHtml}</div>` : ''}</div>`;
  }).join('');
  renderBodyChart();
}
...
function renderBodyChart() {
  ...
  const vals = sorted.map(e => parseFloat(e[metric])); const dates = sorted.map(e => e.date);
  const minV = Math.min(...vals), maxV = Math.max(...vals), range = maxV - minV || 1, W = 280, H = 90, padX = 10, padY = 14;
  const xs = vals.map((v,i) => padX + i / (vals.length - 1) * (W - 2 * padX)); const ys = vals.map(v => H - padY - (v - minV) / range * (H - 2 * padY));
  const polyPts = xs.map((x,i) => `${x},${ys[i]}`).join(' '), areaBot = H - padY, areaPts = `${xs[0]},${areaBot} ${polyPts} ${xs[xs.length-1]},${areaBot}`;
  const circles = vals.map((v,i) => `<circle cx="${xs[i]}" cy="${ys[i]}" r="3.5" fill="var(--accent)"/><text x="${xs[i]}" y="${ys[i]-7}" text-anchor="middle" font-size="8" fill="var(--text-light)">${v}</text>`).join('');
  const labels = { weight:'Вес', waist:'Талия', chest:'Грудь', bicep:'Бицепс', thigh:'Бедро', neck:'Шея', forearm:'Предплечье', glutes:'Ягодицы', calf:'Голень' };
  const unit = metric === 'weight' ? ' кг' : ' см';
  el.innerHTML = `<div style="font-size:11px;color:var(--text-light);font-weight:600;margin-bottom:6px">${labels[metric]}: ${vals[0]}${unit} → ${vals[vals.length-1]}${unit}</div><svg viewBox="0 0 ${W} ${H+12}" style="width:100%;overflow:visible"><defs><linearGradient id="bcg" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="var(--accent)" stop-opacity=".25"/><stop offset="1" stop-color="var(--accent)" stop-opacity="0"/></linearGradient></defs><polygon points="${areaPts}" fill="url(#bcg)"/><polyline points="${polyPts}" fill="none" stroke="var(--accent)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>${circles}<text x="${xs[0]}" y="${H+8}" text-anchor="middle" font-size="8" fill="var(--text-light)">${dates[0].split('-').slice(1).reverse().join('.')}</text><text x="${xs[xs.length-1]}" y="${H+8}" text-anchor="end" font-size="8" fill="var(--text-light)">${dates[dates.length-1].split('-').slice(1).reverse().join('.')}</text></svg>`;
}
