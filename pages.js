...
/* FITSIM — точный символ и размер заголовка редактирования рецепта. */
(function(){
  var originalOpenRecipeEditor = window.openRecipeEditor;
  if (typeof originalOpenRecipeEditor !== 'function') return;
  window.openRecipeEditor = function(id){
    originalOpenRecipeEditor(id);
    setTimeout(function(){
      var titleButton = document.querySelector('#add-recipe-sheet .sheet-title button[onclick*="exitRecipeEditor"]');
      if (titleButton) {
        titleButton.innerHTML = '‹ Изменить рецепт';
        titleButton.style.fontSize = '14px';
        titleButton.style.fontWeight = '700';
        titleButton.style.lineHeight = '1.2';
      }
    }, 100);
  };
})();
