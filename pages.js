...
function renderEncyclopedia() {
  ...
  for (const [cat, exs] of Object.entries(ALL_EXERCISES)) {
    ...
    for (let i = 0; i < exs.length; i++) {
      ...
      // Новый дизайн: простой список с названием, категорией в скобках и сердечком
      out += `
  ...
  // Сброс отступов и прокрутки, чтобы список начинался с самого верха
  if (listEl) {
  ...
  if (filterContainer) {
    ...
    // Добавляем заголовок "Тексты" над фильтрами
...
// Функции раскрытия карточек больше не нужны, но оставляем заглушки для совместимости
function toggleEncCard(id) {
  // Устарело – карточки заменены на простой список
}
