...
function renderDiary() {
  ...
  if (!dl) return;
  if (!diary.length) { dl.innerHTML = '<div class="empty-state">У вас пока нет тренировок</div>'; renderBodyLog(); return; }
  ...
  c.innerHTML = entries.map((e,i) => {
    ...
    if (e.weight !== undefined && e.weight !== null && e.weight !== '') {
      ...
      if (prev && prev.weight !== undefined) {
        const diff = +(Number(e.weight) - Number(prev.weight)).toFixed(1);
    ...
    Object.keys(labels).forEach(k => {
      if (e[k] !== undefined && e[k] !== null && e[k] !== '') {
        let diffHtml = '';
        if (prev && prev[k] !== undefined && prev[k] !== null) {
          const diff = +(Number(e[k]) - Number(prev[k])).toFixed(1);
...
const ENC_CATEGORY_ALIASES = {
  chest: ['грудь','грудные','пекторальные','жим'], back: ['спина','широчайшие','тяга'],
  legs: ['ноги','бедра','квадрицепс','икры'], shoulders: ['плечи','дельты'],
  biceps: ['бицепс'], triceps: ['трицепс'], glutes: ['ягодицы','ягодичные'],
  abs: ['пресс','живот','кор'], forearms: ['предплечья','хват'], cardio: ['кардио','бег','выносливость']
};
function encNormalize(value) {
  return String(value || '').toLowerCase().replace(/ё/g, 'е').replace(/[^а-яa-z0-9]+/g, ' ').trim();
}
function encMatches(e, cat, query) {
  if (!query) return true;
  const haystack = [e.n, ENC_CATEGORY_LABELS[cat], ...(ENC_CATEGORY_ALIASES[cat] || [])].map(encNormalize).join(' ');
  return encNormalize(query).split(/\s+/).every(word => haystack.includes(word));
}
function filterEnc(cat, btn) {
  encCat = cat;
  document.querySelectorAll('.enc-filter-btn').forEach(b => b.classList.toggle('enc-filter-active', b.dataset.cat === cat));
  renderEncyclopedia();
}
window.filterEnc = filterEnc;
function resetEncFilters() {
  encCat = 'all';
  const input = document.getElementById('enc-search');
  if (input) input.value = '';
  renderEncyclopedia();
}
window.resetEncFilters = resetEncFilters;
function clearEncSearch() {
  const input = document.getElementById('enc-search');
  if (!input) return;
  input.value = ''; input.focus(); renderEncyclopedia();
}
window.clearEncSearch = clearEncSearch;
function startEncVoice() {
  if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
    toast('🎙 Голосовой поиск не поддерживается этим браузером'); return;
  }
  const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition;
  const recognition = new Recognition();
  const input = document.getElementById('enc-search');
  const button = document.getElementById('enc-mic-btn');
  recognition.lang = 'ru-RU'; recognition.interimResults = false; recognition.maxAlternatives = 1;
  recognition.onstart = () => { if (button) button.classList.add('listening'); toast('🎙 Слушаю… назови упражнение или мышцы'); };
  recognition.onresult = e => { if (input) { input.value = e.results[0][0].transcript; renderEncyclopedia(); input.focus(); } };
  recognition.onerror = () => toast('Не удалось распознать голос. Попробуй ещё раз');
  recognition.onend = () => { if (button) button.classList.remove('listening'); };
  recognition.start();
}
window.startEncVoice = startEncVoice;
function renderEncyclopedia() {
  ...
  for (const [cat, exs] of Object.entries(ALL_EXERCISES || {})) {
    ...
    exs.forEach((e, i) => {
      const favKey = `${cat}-${i}`;
      if ((encCat === 'favs' && !favExs.has(favKey)) || !encMatches(e, cat, q)) return;
      const isFav = favExs.has(favKey);
      out += `<div class="enc-item"><div class="enc-item-main"><div class="enc-item-name">${e.n}</div><span class="enc-item-category">${ENC_CATEGORY_LABELS[cat] || cat}</span></div><button type="button" class="enc-fav-btn ${isFav ? 'is-fav' : ''}" onclick="toggleFav('${favKey}')" aria-label="${isFav ? 'Убрать из избранного' : 'Добавить в избранное'}">${isFav ? '♥' : '♡'}</button></div>`;
      count++;
    });
  }
  listEl.innerHTML = out || `<div class="enc-empty"><b>Ничего не найдено</b><span>Попробуй другое название, например «жим», «ноги» или «пресс»</span></div>`;
  if (countEl) countEl.textContent = count;
  const filterContainer = document.getElementById('enc-filter-btns');
  if (filterContainer) filterContainer.innerHTML = `<button class="enc-filter-btn ${encCat === 'all' ? 'enc-filter-active' : ''}" data-cat="all" onclick="filterEnc('all',this)">Все</button>${Object.keys(ALL_EXERCISES || {}).map(cat => `<button class="enc-filter-btn ${encCat === cat ? 'enc-filter-active' : ''}" data-cat="${cat}" onclick="filterEnc('${cat}',this)">${ENC_CATEGORY_LABELS[cat] || cat}</button>`).join('')}<button class="enc-filter-btn ${encCat === 'favs' ? 'enc-filter-active' : ''}" data-cat="favs" onclick="filterEnc('favs',this)">♥ Избранное</button>`;
  const clear = document.getElementById('enc-clear-search');
  if (clear) clear.style.visibility = q ? 'visible' : 'hidden';
}
window.renderEncyclopedia = renderEncyclopedia;
function toggleFav(favKey) {
  if (favExs.has(favKey)) favExs.delete(favKey); else favExs.add(favKey);
  localStorage.setItem('fs-fav-exs', JSON.stringify([...favExs]));
  renderEncyclopedia(); toast(favExs.has(favKey) ? 'Добавлено в избранное' : 'Убрано из избранного');
}
