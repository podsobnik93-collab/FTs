...
if (savedRecipes) {
  try {
    ...
    // Добавляем только те, которых ещё нет в базовом списке
    var existingIds = RECIPES.map(r => r.id);
    userRecipes.forEach(function(r) {
...
window.openRecipeSheet = function(recipeId) {
  ...
  sh.innerHTML = `
    ...
    <div style="padding:0 16px 20px;overflow-y:auto;max-height:65vh;">
      ...
      `).join('')}
      <button onclick="addRecipeToMeal('${recipe.id}')" style="margin-top:16px;width:100%;padding:14px;background:var(--accent);color:#fff;border:none;border-radius:14px;font-size:15px;font-weight:700;cursor:pointer;">Добавить в приём пищи</button>
    </div>
  `;
  document.body.appendChild(sh);
  setTimeout(() => { sh.classList.add('show'); ov.classList.add('show'); }, 10);
};
...
window.addRecipeToMeal = function(recipeId) {
  var recipe = RECIPES.find(r => r.id === recipeId);
  if (!recipe) return;
  ...
  setTimeout(() => {
    document.getElementById('nutr-meal-name').value = recipe.name;
    document.getElementById('nutr-kcal').value = recipe.kcal;
    document.getElementById('nutr-protein').value = recipe.protein;
    document.getElementById('nutr-carbs').value = recipe.carbs;
    document.getElementById('nutr-fat').value = recipe.fat;
    toast('Рецепт добавлен: ' + recipe.name);
  }, 400);
};
...
window.openAddRecipeSheet = function() {
  ...
  ov.className = 'sheet-overlay';
  ov.onclick = closeAddRecipeSheet;
  document.body.appendChild(ov);
  ...
  sh.innerHTML = `
    ...
    <div style="padding:16px 20px 24px;overflow-y:auto;max-height:70vh;">
      ...
      <button onclick="saveNewRecipe()" style="width:100%;padding:14px;background:var(--accent);color:#fff;border:none;border-radius:14px;font-size:15px;font-weight:700;cursor:pointer;">Сохранить рецепт</button>
    </div>
  `;
  document.body.appendChild(sh);
  setTimeout(() => { sh.classList.add('show'); ov.classList.add('show'); }, 10);
