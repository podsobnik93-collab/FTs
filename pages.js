...
function goTo(id, title) {
  ...
  if (id === 'diary') { renderDiary(); renderProgress(); requestAnimationFrame(() => setTimeout(renderBodyLog, 60)); }
...
function renderBodyLog() {
  ...
  if (!c) {
    if (!bodyLogRetryTimer) bodyLogRetryTimer = setTimeout(() => { bodyLogRetryTimer = null; renderBodyLog(); }, 80);
    return;
  }
  refreshBodyStorage();
  ...
  const lastEl = document.getElementById('diary-last-body');
  const chartCard = document.getElementById('body-chart-card');
  if (lastEl) {
    const latest = entries[0];
    if (!latest) lastEl.innerHTML = '<div class="diary-empty">Добавьте вес или замеры, чтобы отслеживать прогресс</div>';
    else {
      const parts = [];
      if (latest.weight !== undefined && latest.weight !== null && latest.weight !== '') parts.push(`<b>${latest.weight} кг</b><span>вес</span>`);
      const measurements = Object.keys(labels).filter(k => latest[k] !== undefined && latest[k] !== null && latest[k] !== '');
      if (measurements.length) parts.push(`<b>${measurements.length}</b><span>замеров</span>`);
      lastEl.innerHTML = `<div class="diary-last-date">Последняя запись · ${formatBodyDate(latest.date)}</div><div class="diary-last-metrics">${parts.join('')}</div>`;
    }
  }
  ...
  c.innerHTML = entries.map((e,i) => {
    ...
    const dateStr = formatBodyDate(e.date);
