...
listEl.innerHTML = items.map(function(e) {
      return '<div class="meal-item-wrap" data-id="' + e.id + '">' +
        '<div class="meal-item-delete-bg">Удалить</div>' +
        '<div class="meal-item" onclick="editNutrEntry(\'' + e.id + '\')">' +
          ...
          '</div>' +
          // Оставляем обычный крестик для тех, кто не догадается свайпнуть
...
function attachMealSwipeGestures() {
  document.querySelectorAll('.meal-item-wrap').forEach(function(wrap) {
    ...
    var startY = 0;
    var dragging = false;
    var isHorizontal = null; // null - неизвестно, true - свайп влево/вправо, false - скролл
    ...
    item.addEventListener('touchmove', function(e) {
      ...
      if (isHorizontal) {
        // Двигаем только влево (dx < 0), максимум на 80px (или больше, если хотим показать фон)
        var translateX = dx < 0 ? Math.max(dx, -90) : 0;
    ...
    item.addEventListener('touchend', function(e) {
      ...
      if (dx < -50) { // если свайпнули влево достаточно сильно
        item.style.transform = 'translateX(-100vw)';
        var id = wrap.getAttribute('data-id');
        setTimeout(function() {
          removeNutrEntryById(id);
        }, 250);
      } else {
