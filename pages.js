...
window.openAddRecipeSheet = function() {
  ...
  sh.className = 'bottom-sheet';
  sh.style.maxHeight = '90vh';
  sh.innerHTML = `
    <div class="sheet-handle"></div>
    <div class="sheet-title">➕ Добавить рецепт</div>
    <div style="padding:0 20px 24px;overflow-y:auto;max-height:76vh;">
      <div style="display:grid;grid-template-columns:minmax(0,1fr) 82px;gap:10px;align-items:end;">
        ...
        <div class="sheet-field">
          <label>Смайлик</label>
          <input type="text" id="new-recipe-emoji" value="🍽️" maxlength="8" aria-label="Смайлик рецепта" style="width:100%;padding:12px 8px;border-radius:12px;border:1px solid var(--border);background:var(--input-bg);color:var(--text);font-size:20px;outline:none;text-align:center;" />
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">
        <div class="sheet-field">
          <label>Ккал</label>
          <input type="number" inputmode="decimal" min="0" id="new-recipe-kcal" placeholder="450" style="width:100%;padding:12px 14px;border-radius:12px;border:1px solid var(--border);background:var(--input-bg);color:var(--text);font-size:15px;outline:none;" />
        </div>
        <div class="sheet-field">
          <label>Время (мин)</label>
          <input type="number" inputmode="numeric" min="0" id="new-recipe-time" placeholder="25" style="width:100%;padding:12px 14px;border-radius:12px;border:1px solid var(--border);background:var(--input-bg);color:var(--text);font-size:15px;outline:none;" />
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:18px;">
        <div class="sheet-field">
          <label>Белки (г)</label>
          <input type="number" inputmode="decimal" min="0" id="new-recipe-protein" placeholder="48" style="width:100%;padding:12px 10px;border-radius:12px;border:1px solid var(--border);background:var(--input-bg);color:var(--text);font-size:15px;outline:none;" />
        </div>
        <div class="sheet-field">
          <label>Жиры (г)</label>
          <input type="number" inputmode="decimal" min="0" id="new-recipe-fat" placeholder="6" style="width:100%;padding:12px 10px;border-radius:12px;border:1px solid var(--border);background:var(--input-bg);color:var(--text);font-size:15px;outline:none;" />
        </div>
        <div class="sheet-field">
          <label>Углеводы (г)</label>
          <input type="number" inputmode="decimal" min="0" id="new-recipe-carbs" placeholder="42" style="width:100%;padding:12px 10px;border-radius:12px;border:1px solid var(--border);background:var(--input-bg);color:var(--text);font-size:15px;outline:none;" />
        </div>
      </div>
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin:2px 0 10px;">
        <div style="font-size:15px;font-weight:800;color:var(--text);">Ингредиенты</div>
        <button type="button" onclick="window.addRecipeIngredientRow()" style="padding:8px 11px;background:var(--input-bg);color:var(--accent);border:1px solid var(--border);border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;">＋ Добавить</button>
      </div>
...
// Добавляет строку «ингредиент + граммовка» в форму рецепта.
window.addRecipeIngredientRow = function(name, grams) {
  ...
  row.innerHTML = '<input class="new-recipe-ingredient-name" type="text" placeholder="Ингредиент" value="' + (name || '').replace(/"/g, '&quot;') + '" style="width:100%;padding:11px 12px;border-radius:11px;border:1px solid var(--border);background:var(--input-bg);color:var(--text);font-size:14px;outline:none;" />' +
    '<input class="new-recipe-ingredient-grams" type="number" inputmode="decimal" min="0" placeholder="грамм" value="' + (grams || '') + '" style="width:100%;padding:11px 8px;border-radius:11px;border:1px solid var(--border);background:var(--input-bg);color:var(--text);font-size:14px;outline:none;text-align:center;" />' +
    '<button type="button" aria-label="Удалить ингредиент" title="Удалить" style="width:36px;height:36px;padding:0;border:1px solid var(--border);border-radius:10px;background:var(--input-bg);color:#ef4444;font-size:21px;line-height:1;cursor:pointer;">×</button>';
...
window.closeAddRecipeSheet = function() {
  ...
  if (ov) ov.classList.remove('show');
  if (sh) { sh.classList.remove('show'); sh.style.transform = 'translateY(100%)'; }
  setTimeout(function(){ if (ov) ov.remove(); if (sh) sh.remove(); }, 300);
};

// Сохранение нового рецепта
window.saveNewRecipe = function() {
  var name = document.getElementById('new-recipe-name')?.value.trim();
  var emoji = document.getElementById('new-recipe-emoji')?.value.trim() || '🍽️';
  var kcal = parseFloat(document.getElementById('new-recipe-kcal')?.value) || 0;
  var minutes = parseFloat(document.getElementById('new-recipe-time')?.value) || 0;
  ...
  var ingredients = [];
  var rows = document.querySelectorAll('#new-recipe-ingredients > div');
  for (var i = 0; i < rows.length; i++) {
    var ingredientName = rows[i].querySelector('.new-recipe-ingredient-name')?.value.trim();
    var gramsRaw = rows[i].querySelector('.new-recipe-ingredient-grams')?.value;
    if (!ingredientName && !gramsRaw) continue;
    var grams = parseFloat(gramsRaw);
    if (!ingredientName) { toast('Введите название ингредиента'); return; }
    if (!Number.isFinite(grams) || grams <= 0) { toast('Укажите граммовку для: ' + ingredientName); return; }
    ingredients.push({ name: ingredientName, grams: Math.round(grams * 10) / 10 });
  }
  ...
  var newRecipe = {
    ...
    name: name,
    emoji: emoji,
    time: minutes > 0 ? Math.round(minutes) + ' мин' : '0 мин',
    kcal: Math.round(kcal),
