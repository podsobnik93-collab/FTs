    ...
    listEl.innerHTML = items.map(function(e) {
      return '<div class="meal-item">' +
        '<div class="meal-item-info">' +
          ...
          '<div class="meal-item-meta">' + Math.round(e.kcal) + ' ккал · Б:' + (+parseFloat(e.protein).toFixed(1)) + ' · У:' + (+parseFloat(e.carbs).toFixed(1)) + ' · Ж:' + (+parseFloat(e.fat).toFixed(1)) + '</div>' +
        '</div>' +
        '<button onclick="removeNutrEntryById(\'' + e.id + '\')" class="meal-item-del">✕</button>' +
      '</div>';
    }).join('');
  });
...
function applyFoodItem(name) {
  ...
  closeAllSheets();
  // Автосохранение в выбранный приём пищи
  addNutrEntry(window._currentMealType || 'breakfast');
  toast('✅ ' + item.n + ' добавлено');
}
