...
function updateCtaLabel(){
  ...
  if(currentPage === 'nutrition' || currentPage === 'home' || currentPage === 'calendar' || currentPage === 'ai' || currentPage === 'diary' || currentPage === 'encyclopedia'){
...
function showStatsTip(key) {
  ...
  if (factor !== 1) tdeeBody += '\n\nПоправка стажа (' + expLbl + (coachLbl !== 'самостоятельно' ? ', ' + coachLbl : '') + '): ' + (delta > 0 ? '+' : '') + delta + ' ккал\nОпытные атлеты сжигают больше; новички — чуть меньше, пока тело адаптируется.';
  var tips = {
  ...
  var tip = tips[key];
  if (!tip) return;
  document.getElementById('stats-tip-overlay')?.remove();
  var overlay = document.createElement('div');
  overlay.id = 'stats-tip-overlay';
  overlay.className = 'stats-tip-overlay';
  var panel = document.createElement('section');
  panel.className = 'stats-tip-panel';
  panel.setAttribute('role','dialog');
  panel.setAttribute('aria-modal','true');
  panel.innerHTML = '<button class="stats-tip-close" type="button" aria-label="Закрыть">×</button><h3></h3><div class="stats-tip-body"></div>';
  panel.querySelector('h3').textContent = tip.title;
  panel.querySelector('.stats-tip-body').textContent = tip.body;
  panel.querySelector('.stats-tip-close').onclick = function(){ overlay.remove(); };
