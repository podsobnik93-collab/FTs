...
function showCalDay(dateStr) {
  ...
  if(dayNutr.length&&dayDiary.length){const spent=dayDiary.reduce((s,d)=>s+(Number(d.kcal)||0),0),balance=totalKcal-spent;html+=`<div class="cal-day-card cal-balance"><div class="cal-day-card-title">⚖️ Баланс дня</div><div><span>Потреблено</span><b>${Math.round(totalKcal)} ккал</b></div><div><span>Потрачено</span><b>− ${spent} ккал</b></div><hr><div><strong>Итого</strong><strong class="${balance>0?'plus':'minus'}">${balance>0?'+':''}${Math.round(balance)} ккал</strong></div></div>`}
