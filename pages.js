...
function renderCalendar() {
  ...
  const workoutDays={},foodDays={},waterDays={};
  ...
  for(let d=1;d<=daysInMonth;d++){
    ...
    const wt=workoutDays[dateStr],food=foodDays[dateStr],water=waterDays[dateStr],planned=plannedDays[dateStr],isPlanned=planned&&!planned.rest;
    const marks=[water?'water':'',food?'food':'',wt||isPlanned?'workout':''].filter(Boolean).map(x=>`<i class="cal-mark ${x}"></i>`).join('');
  ...
  const anyData=Object.keys(workoutDays).length||Object.keys(foodDays).length||Object.keys(waterDays).length||Object.keys(plannedDays).length;
