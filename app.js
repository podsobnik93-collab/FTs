// app.js — полная логика FitSim (оригинальный скрипт, адаптированный для отдельного файла)
// Все глобальные переменные и функции оставлены как есть, привязаны к window.

// ==================== ИСПРАВЛЕННАЯ ВЕРСИЯ ====================
// Все данные, объединённая база упражнений, редактирование тренировки, сохранение состояния
...
function closeAllSheets() {
  ...
  document.getElementById('body-sheet').classList.remove('show');
  document.getElementById('edit-ex-sheet').classList.remove('show');
  document.getElementById('add-ex-sheet').classList.remove('show');
}
...
function goTo(id, title) {
  ...
  if (id==='nutrition') { renderNutrEntries(); renderDietPrefs(); }  // добавили отрисовку предпочтений
...
function loadProfileToForm() {
  ...
  renderDietPrefs(); // всё ещё вызывается для формы, но карточка в профиле удалена — это не мешает
}
...
function renderSavedWts(){
  ...
  if(!savedWts.length){card.style.display='none';return;}
  card.style.display='block'; if(cnt)cnt.textContent=savedWts.length+' шаблонов';
...
function renderWaterWeek(){
  ...
  for(let i=6;i>=0;i--){
    ...
    const inHistory=waterHistory.find(h=>h.date===dateStr);
    const isToday=i===0;
    const amount=isToday?waterData.amount:(inHistory?inHistory.amount:0);
    const weekDay=d.toLocaleDateString('ru-RU',{weekday:'short'});
    days.push({label:weekDay,amount,isToday});
  }
  const goal=profile?(profile.weight?Math.round(profile.weight*30):2000):2000;
