...
// UPD: добавлен стаж тренировок (trainingMonths) и авто-уровень
...
function loadProfileToForm() {
  ...
  document.getElementById('p-training-months').value = profile.trainingMonths || 0;
  if (window.updateTrainingExpLabel) window.updateTrainingExpLabel();
  renderDietPrefs();
}
...
function saveProfile() {
  ...
  profile = {
    ...
    trainingMonths: parseInt(document.getElementById('p-training-months').value) || oldProfile.trainingMonths || 0,
...
// ==================== СТАЖ И УРОВЕНЬ (новая функция) ====================
window.updateTrainingExpLabel = function() {
  var inp = document.getElementById('p-training-months');
  var el  = document.getElementById('p-exp-label');
  if (!inp || !el) return;
  var m = parseInt(inp.value) || 0;
  var label = m === 0 ? 'Не указан' : m <= 3 ? '🌱 Новичок' : m <= 12 ? '🌿 Начинающий' :
              m <= 36 ? '💪 Средний' : m <= 72 ? '🔥 Продвинутый' : '⚡ Эксперт';
  var color = m === 0 ? 'var(--text-light)' : m <= 12 ? '#22c55e' :
              m <= 36 ? '#a78bfa' : m <= 72 ? '#f59e0b' : '#ef4444';
  el.textContent = label + (m > 0 ? ' (' + m + ' мес.)' : '');
