...
function getProfile() {
  try { return JSON.parse(localStorage.getItem('fs-profile') || '{}'); }
  catch(e) { return {}; }
}
...
function getRecentWorkouts() {
  try {
    var diary = JSON.parse(localStorage.getItem('fs-diary') || '[]');
...
function getWaterToday() {
  try {
    var w = JSON.parse(localStorage.getItem('fs-water') || '{}');
    return w.amount || 0;
  } catch(e) { return 0; }
}
...
function persistChatHistory() {
  try { localStorage.setItem('fs-ai-chat-history', JSON.stringify(aiChatHistory.slice(-50))); }
  catch (e) {}
}
...
function setAiPersonality(p) {
  ...
  localStorage.setItem('fs-ai-personality', p);
...
function saveGroqKey() {
  ...
  try {
    localStorage.setItem('fs-groq-key', key);
  } catch(e) {}
  // Mask the input
  inp.type = 'password';
  if (statusEl) {
...
function initAiPage() {
  ...
  // Restore personality
  try {
    var saved = localStorage.getItem('fs-ai-personality');
