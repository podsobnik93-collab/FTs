...
window.aiGeneratePlan = async function() {
  ...
  var _planAgeNote = userAge < 18
    ? ' (подросток: без дефицита калорий, акцент на технику, без тяжёлой осевой нагрузки)'
    : userAge > 55
    ? ' (55+: норма белка 2.0–2.4 г/кг, упражнения на баланс, умеренные веса)'
    : '';
  ...
  try {
    ...
    if (key) {
      ...
      var usr = 'Составь 4-недельную программу. Цель: ' + gl + ', уровень: ' + aiLevel + ', стаж: ' + expLabel + ', дней в неделю: ' + days + ', время: ' + dur + ' мин. Оборудование: ' + (aiEquip.join(', ') || 'зал') + '.' + (restr ? ' Ограничения: ' + restr : '') + ' Клиент: ' + userGender + ', ' + userAge + ' лет' + _planAgeNote + ', вес ' + userWeight + ' кг, рост ' + userHeight + ' см. ' + userGoalW;
...
window.aiAsk = async function(prefill) {
  ...
  if (profile.weight) {
    ...
    var _chatAge = Number(profile.age) || 25;
    var _ageNote = _chatAge < 18 ? ' (подросток, дефицит калорий противопоказан)' :
                   _chatAge > 55 ? ' (55+: повышенная норма белка, умеренный дефицит/профицит)' : '';
    profileInfo = 'Пользователь: ' + (profile.gender === 'female' ? 'женщина' : 'мужчина') +
      ', ' + _chatAge + ' лет' + _ageNote + ', вес ' + profile.weight + ' кг, рост ' + (profile.height || 175) +
  ...
  try {
    ...
    if (key) {
      ...
      if (tl.includes('белок') || tl.includes('протеин')) {
        var _pAge = Number(profile.age) || 25;
        var _pPerKg = _pAge > 55
          ? (profile.goal === 'gain' ? 2.4 : 2.0)   // пожилым больше белка для сохранения мышц
          : (_pAge < 18
              ? (profile.goal === 'gain' ? 2.0 : 1.6) // подросткам умеренно
              : (profile.goal === 'gain' ? 2.2 : 1.8));
        var proteinGoal = weight * _pPerKg;
        var _pNote = _pAge > 55 ? ' (повышена норма для 55+: защита мышц)' : (_pAge < 18 ? ' (норма для подростков)' : '');
        reply = '**Белок:** ориентир ' + proteinGoal.toFixed(1) + ' г/день (' + _pPerKg.toFixed(1) + ' г/кг)' + _pNote + '. При похудении держи верхнюю границу.';
      } else if (tl.includes('дефицит') || tl.includes('калори')) {
        var _aiAge = Number(profile.age) || 25;
        var _aiBase = 10 * weight + 6.25 * (profile.height || 175) - 5 * _aiAge;
        var bmr = (profile.gender === 'female') ? _aiBase - 161 : _aiBase + 5;
        if (_aiAge > 60) bmr = Math.round(bmr * (1 - (Math.min(_aiAge - 60, 30) / 300)));
        var tdee = Math.round(bmr * (profile.activity || 1.55));
        var _ageDeficit = _aiAge > 55 ? 300 : 400;
        var _ageSurplus = _aiAge > 55 ? 200 : 300;
        var deficit = profile.goal === 'loss' ? _ageDeficit : (profile.goal === 'gain' ? -_ageSurplus : 0);
        var rec = Math.max(1200, Math.round(tdee - deficit));
        reply = '**Калорийность:** твой TDEE ~ ' + tdee + ' ккал. Для цели "' + (profile.goal || 'поддержание') + '" рекомендую ~ ' + rec + ' ккал/день.' + (_aiAge > 55 ? ' (норма скорректирована для возраста 55+)' : '');
